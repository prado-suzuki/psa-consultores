...
| Etapa | Alterações |
|---|---|
| Migration SQL | Adicionada coluna `ambiente` em `cliente` e `contribuinte` (default `'prod'`). Dados migrados de `cliente_dev`, `contribuinte_dev`, `participante_dev`. Triggers e functions atualizados. Tabelas `_dev` removidas. Valores renomeados de `'producao'`→`'prod'` e `'desenvolvimento'`→`'dev'`. |
