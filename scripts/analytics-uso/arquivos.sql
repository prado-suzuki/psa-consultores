...
--   * Todo campo tipado `taxa` protegido com IFNULL(..., 0). Sem isso,
--     `/arquivos?usuario=<pessoa so-de-API>` devolve totais.taxaErro = null
--     (0 linhas em `humanos`) e quebra a tela -- reproduzido com 9 das 22
--     pessoas do dropdown, que tem atividade na API e nenhum arquivo.
...
SELECT TO_JSON_STRING(STRUCT(
  ...
  (
    SELECT AS STRUCT
      ...
      IFNULL(ROUND(SAFE_DIVIDE(
        COUNTIF(status_processamento = 'ERRO'), COUNT(*)), 4), 0)
                                                                AS taxaErro,
  ...
  ARRAY(
    SELECT AS STRUCT
      ...
      IFNULL(ROUND(SAFE_DIVIDE(COUNT(*), SUM(COUNT(*)) OVER ()), 4), 0)
                                                                AS pct
    FROM humanos
  ...
  STRUCT(
    ARRAY(
      SELECT AS STRUCT
        ...
        COUNTIF(status_processamento = 'SUCESSO')               AS enviados,
        COUNTIF(status_processamento = 'ERRO')                  AS erros,
        IFNULL(ROUND(SAFE_DIVIDE(COUNTIF(status_processamento = 'ERRO'), COUNT(*)), 4), 0)
                                                                AS taxaErro,
        ...
        IFNULL(ROUND(SAFE_DIVIDE(
          ...
          COUNTIF(status_processamento = 'SUCESSO')), 4), 0)    AS participacaoAutomacao,
