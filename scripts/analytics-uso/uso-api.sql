...
--
-- FIX 2026-08-07 (SPEC-endpoints-analytics-uso.md, secao 8):
--   * Todo campo tipado `taxa` ou `num` (SAFE_DIVIDE, AVG, APPROX_QUANTILES)
--     agora esta protegido com IFNULL(..., 0). Sem isso, um recorte sem
--     nenhuma linha (periodo vazio, @usuario sem chamadas de API, etc.) faz o
--     BigQuery devolver NULL onde o zod do front exige numero, e a tela
--     inteira quebra com "A API retornou dados incompativeis com o dashboard".
--   * UNICA excecao mantida nullable de proposito: `taxaRetencao` em
--     gerencial.porMes/porClusterMes. E null no primeiro mes da serie porque
--     nao existe base de retencao anterior (usuariosBaseRetencao = 0) -- essa
--     divisao e indefinida por definicao, nao um buraco de dado.
WITH origem AS (
  SELECT
    normalized_path,
...
metricas_mes_org AS (
  SELECT
    ...
    COUNT(*) AS chamadas,
    IFNULL(ROUND(SAFE_DIVIDE(COUNT(*), COUNT(DISTINCT h.nome_responsavel)), 1), 0)
      AS chamadas_por_usuario,
    COUNT(DISTINCT h.agrupado_ferramentas) AS ferramentas_ativas,
    IFNULL(ROUND(SAFE_DIVIDE(COUNTIF(NOT h.is_erro), COUNT(*)), 4), 0) AS taxa_sucesso
  FROM humanos h
...
SELECT TO_JSON_STRING(STRUCT(
  ...
  (
    SELECT AS STRUCT
      ...
      IFNULL(ROUND(SAFE_DIVIDE(COUNTIF(is_erro), COUNT(*)), 4), 0)
                                                                AS taxaErro,
      IFNULL(ROUND(SAFE_DIVIDE(COUNTIF(status_code BETWEEN 500 AND 599), COUNT(*)), 4), 0)
                                                                AS taxa5xx,
      IFNULL(ROUND(AVG(duration_ms), 1), 0)                     AS latMediaMs,
      IFNULL(APPROX_QUANTILES(duration_ms, 100)[OFFSET(50)], 0) AS latP50Ms,
      IFNULL(APPROX_QUANTILES(duration_ms, 100)[OFFSET(95)], 0) AS latP95Ms,
  ...
  STRUCT(
    ...
    ARRAY(
      SELECT AS STRUCT
        ...
        -- NAO envolver em IFNULL: nulo aqui e legitimo (primeiro mes da serie
        -- nao tem base de retencao anterior). O front trata esse taxa? como
        -- opcional de proposito -- ver schemas.ts:107.
    ...
    ARRAY(
      SELECT AS STRUCT
        ...
        IFNULL(r.usuarios_retidos, 0)                           AS usuariosRetidos,
        IFNULL(r.usuarios_base, 0)                              AS usuariosBaseRetencao,
        -- idem: taxaRetencao nula e legitima, nao envolver em IFNULL.
        ROUND(SAFE_DIVIDE(r.usuarios_retidos, r.usuarios_base), 4)
                                                                AS taxaRetencao,
        m.chamadas                                              AS chamadas,
        m.chamadas_por_usuario                                  AS chamadasPorUsuario,
    ...
    ARRAY(
      SELECT AS STRUCT
        ...
        COUNT(DISTINCT nome_responsavel)                        AS usuariosAtivos,
        COUNT(*)                                                AS chamadas,
        IFNULL(ROUND(SAFE_DIVIDE(COUNT(DISTINCT nome_responsavel),
              (SELECT COUNT(DISTINCT nome_responsavel) FROM humanos)), 4), 0)
                                                                AS coberturaUsuarios,
        IFNULL(ROUND(SAFE_DIVIDE(COUNTIF(NOT is_erro), COUNT(*)), 4), 0)
                                                                AS taxaSucesso
      FROM humanos
      GROUP BY agrupado_ferramentas
    ...
    ARRAY(
      SELECT AS STRUCT
        ...
        COUNT(*)                                                AS chamadas,
        IFNULL(ROUND(SAFE_DIVIDE(COUNT(DISTINCT h.nome_responsavel),
              ANY_VALUE(u.usuarios)), 4), 0)                    AS coberturaUsuarios,
        IFNULL(ROUND(SAFE_DIVIDE(COUNTIF(NOT h.is_erro), COUNT(*)), 4), 0)
                                                                AS taxaSucesso
      FROM humanos h
