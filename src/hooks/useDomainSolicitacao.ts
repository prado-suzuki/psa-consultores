...
export function useDomainSolicitacao(clienteId: string | null) {
  ...
  const encerrarSolicitacao = useMutation({
    ...
    onSuccess: () => {
       ...
       * mandar "recebemos e conferimos" nesse caso seria afirmar algo falso. A
       * borda repete essa checagem porque é ela que pode ser chamada de fora;
       * aqui a guarda existe só para não gerar chamada condenada em todo
       * encerramento de rascunho.
       *
