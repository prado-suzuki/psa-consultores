...
function buildTaskTree(tasks: OrgTask[]) {
  ...
  // Por prazo em toda a profundidade: raízes do projeto, filhas, netas. Cada
  // nível ordena entre irmãs — ver `compararTarefasPorPrazo` para a regra (sem
  // prazo na frente, título desempatando data igual).
  const sortNodes = (items: ProjetosTarefasTaskNode[]) => {
    items.sort((a, b) => compararTarefasPorPrazo(a.task, b.task));
