...
describe('modo Classificar — cadastrar a partir do arquivo', () => {
  it('cria a pessoa e vincula o arquivo aberto a ela (1:1)', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Cadastrar e voltar ao balde/ }));
  ...
  it('recusa mandar georreferenciamento para pessoa (a exceção conhecida)', () => {
    ...
    expect(mocks.toast.error).toHaveBeenLastCalledWith(
      'Documentos de georreferenciamento precisam estar vinculados a uma matrícula.',
    );
    expect(mocks.pessoaMutate).not.toHaveBeenCalled();
  });
});
...
describe('modo Classificar — apontar para quem já existe', () => {
  it('vincula sem criar cadastro novo', async () => {
    const user = userEvent.setup();
    renderClassificar();
    await user.click(screen.getByRole('button', { name: 'Já cadastrado' }));
    await user.click(screen.getByRole('button', { name: /Escolha a pessoa/ }));
    await user.click(screen.getByRole('button', { name: /Pessoas Físicas/ }));
    await user.click(screen.getByRole('button', { name: 'Antônio Lima' }));
    await user.click(screen.getByRole('button', { name: /Vincular e voltar ao balde/ }));
