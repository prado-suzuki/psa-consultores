...
export function GaleriaModelos({ modelos, isLoading, onSelect, onNovo }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-osg-300/60 bg-superficie-cartao p-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {/* Card "Novo modelo" */}
      <button
        type="button"
        onClick={onNovo}
      ...
      {modelos.length === 0 ? (
        ...
        modelos.map((m) => (
          <button
            ...
            className={cn(
              'group relative flex min-h-[8.5rem] flex-col rounded-2xl border border-osg-300/60 bg-field p-4 text-left shadow-[0_1px_2px_hsl(var(--osg-700)/0.06)] transition-all hover:-translate-y-0.5 hover:border-osg-moss/50 hover:shadow-md',
              !m.ativo && 'opacity-60',
            )}
          >
