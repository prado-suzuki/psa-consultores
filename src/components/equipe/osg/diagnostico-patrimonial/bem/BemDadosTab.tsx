...
const STATUS = ['Pendente', 'Em análise', 'Aprovado', 'Aprovado para 2ª Instancia', 'Integralizado', 'Recusado', 'Não se aplica'];
...
export function BemDadosTab({ draft, onChange, pessoas, ...matriculaProps }: Props) {
  ...
  return <>
    <FieldSection number={next()} title="Identificação"><div className="grid grid-cols-1 gap-3 md:grid-cols-3">
      <Field label="Referência DP" required><Input value={draft.referencia_dp} onChange={(e) => set('referencia_dp', e.target.value)} placeholder="ex: IR-01" className={`${fieldCls} font-mono`} /></Field>
...
function Money({ label, required, value, onChange }: { label: string; required?: boolean; value: string; onChange: (value: string) => void }) { return <Field label={label} required={required}><CurrencyInput value={value} onChange={onChange} className={`${fieldCls} font-mono`} /></Field>; }
