...
const GEORREFERENCIAMENTO_OPTIONS = ['Sim', 'Não', 'Parcial', 'Em processo'];
...
export function MatriculaModal({
  ...
  const handleSave = () => {
    ...
    upsert.mutate({ values, original: matricula, titular }, { onSuccess: () => onClose() });
  };
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Select
                          value={draft.uf_imovel || undefined}
                          onValueChange={(v) => setField('uf_imovel', v)}
                        >
                          <SelectTrigger className={fieldCls}>
                            <SelectValue placeholder="—" />
                          </SelectTrigger>
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        <SelectTrigger className={fieldCls}>
                          <SelectValue
                            placeholder={
                              matriculasAnterioresPossiveis.length ? 'Selecione...' : 'Nenhuma'
                            }
                          />
                        </SelectTrigger>
                        <SelectContent>
              ...
              <TabsContent value="titulares" className="mt-0 focus-visible:ring-0">
                {isEdit && matricula ? (
                  ...
                  // Na criação a matrícula ainda não existe: define-se o titular
                  // inicial, gravado junto na mesma transação (criar_matricula_com_titular).
                    ...
                    <div className="rounded-md border border-osg-moss/20 bg-osg-moss/[0.04] p-4">
                      {semPessoas ? (
                        <p className="text-xs text-amber-600">
                          ...
                          (ou selecione um cliente) antes de criar a matrícula.
                        </p>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div className="space-y-1.5">
...
function ImpedimentosPanel({ matriculaId, areaUnidade, pessoasCliente }: ImpedimentosPanelProps) {
  ...
  return (
    <FieldSection
      ...
      title="Impedimentos"
      hint={
        !isLoading && impedimentos.length > 0 ? `${impedimentos.length} registro(s)` : undefined
      }
    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    {TIPO_IMPEDIMENTO_OPTIONS.map((o) => (
                      <SelectItem key={o} value={o}>
                        {o}
                      </SelectItem>
                    ))}
...
function ImpedimentoRowItem({
  impedimento,
  areaUnidade,
  isEditing,
  onEdit,
  onDelete,
}: ImpedimentoRowItemProps) {
      ...
      <div className="flex items-start gap-2">
        <div className="flex-1 min-w-0 space-y-1">
          ...
          <div className="flex gap-3 text-xs text-muted-foreground flex-wrap">
            {impedimento.vlr != null && (
              <span>
                Valor: <span className="font-mono">{formatVlr(impedimento.vlr)}</span>
              </span>
            )}
            {impedimento.area_afetada != null && (
              <span>
                ...
                <span className="font-mono">
                  {impedimento.area_afetada} {formatAreaUnidade(areaUnidade)}
                </span>
              </span>
            )}
            {impedimento.data_constituicao && (
              <span>
                Constituído: {new Date(impedimento.data_constituicao).toLocaleDateString('pt-BR')}
              </span>
            )}
            {impedimento.data_validade && (
              <span>
                Validade: {new Date(impedimento.data_validade).toLocaleDateString('pt-BR')}
              </span>
            )}
          </div>
        </div>
