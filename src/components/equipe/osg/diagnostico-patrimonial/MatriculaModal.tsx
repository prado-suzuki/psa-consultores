    ...
    <Dialog open={open} onOpenChange={(value) => !value && requestClose()}>
      <DialogContent className="flex max-h-[90vh] max-w-4xl flex-col gap-0 overflow-visible p-0 sm:[clip-path:none]">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex min-h-0 flex-1 flex-col">
          ...
          <DialogFooter className="shrink-0 rounded-b-lg border-t border-osg-100 bg-background px-6 py-3.5"><Button variant="outline" onClick={requestClose} disabled={upsert.isPending}>Cancelar</Button><Button onClick={handleSave} disabled={upsert.isPending || (!isEdit && semPessoas)} className="gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90">{upsert.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin" />}{rascunhoExterno?.rotuloSalvar ?? (isEdit ? 'Salvar alterações' : 'Cadastrar matrícula')}</Button></DialogFooter>
        </Tabs>
