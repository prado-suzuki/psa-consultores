...
import { Campo, FieldSection, fieldCls, labelCls, switchBoxCls, textareaCls } from '@/components/equipe/osg/formKit';
...
export function MatriculaDadosTab({ draft, onChange, bemTipo, matricula, matriculasDoBem }: Props) {
  ...
  return <>
    <FieldSection number={next()} title="Identificação"><div className="space-y-3"><div className={`${formGridCls(4)} gap-3`}>
      <Campo rotulo="Nº da matrícula" required campo="numero"><Input value={draft.numero} onChange={(e) => set('numero', e.target.value)} className={`${fieldCls} font-mono`} /></Campo>
      <Campo rotulo="Tipo do imóvel"><Select value={draft.tipo_bem || undefined} onValueChange={(v: 'IR' | 'IB') => set('tipo_bem', v)}><SelectTrigger className={fieldCls}><SelectValue placeholder="—" /></SelectTrigger><SelectContent><SelectItem value="IR"><span className="mr-2 font-mono">IR</span>Imóvel Rural</SelectItem><SelectItem value="IB"><span className="mr-2 font-mono">IB</span>Imóvel Urbano</SelectItem></SelectContent></Select></Campo>
      <Campo rotulo="Livro"><Input value={draft.livro} onChange={(e) => set('livro', e.target.value)} className={fieldCls} /></Campo>
      <Campo rotulo="Folha"><Input value={draft.folha} onChange={(e) => set('folha', e.target.value)} className={fieldCls} /></Campo>
      <Campo rotulo="Data"><DateFieldWithInput value={draft.data_matricula} onChange={(v) => set('data_matricula', v)} /></Campo>
    </div><Campo rotulo="Cartório" required campo="cartorio_id"><CartorioSelect value={draft.cartorio_id} onChange={(v) => set('cartorio_id', v)} /></Campo></div></FieldSection>
    <FieldSection number={next()} title="Localização e áreas" hint={draft.area_unidade === 'ha_m2' ? 'áreas em ha e m² — inteiro = ha, decimais = m² (123,1234 = 123 ha e 1.234 m²); trocar a unidade converte o valor' : `áreas em ${formatAreaUnidade(draft.area_unidade)} — trocar a unidade converte o valor`}>
      <div className="space-y-3"><div className={`${formGridCls(3)} gap-3`}><div data-campo="municipio_imovel" className={`space-y-1.5 ${formSpanCls(2)}`}><Label className={labelCls}>Município<RequiredMark /></Label><Input value={draft.municipio_imovel} onChange={(e) => set('municipio_imovel', e.target.value)} className={fieldCls} /></div>
        <Campo rotulo="UF" required campo="uf_imovel"><Select value={draft.uf_imovel || undefined} onValueChange={(v) => set('uf_imovel', v)}><SelectTrigger className={fieldCls}><SelectValue placeholder="—" /></SelectTrigger><SelectContent>{UF_STATES.map((uf) => <SelectItem key={uf} value={uf}>{uf}</SelectItem>)}</SelectContent></Select></Campo></div>
      <div className={`${formGridCls(4)} gap-3`}><Campo rotulo="Unidade"><Select value={draft.area_unidade} onValueChange={trocarUnidade}><SelectTrigger className={fieldCls}><SelectValue /></SelectTrigger><SelectContent>{UNIDADES.map((unit) => <SelectItem key={unit.value} value={unit.value}>{unit.label}</SelectItem>)}</SelectContent></Select></Campo>
    ...
    <FieldSection number={next()} title="Histórico e descrição"><div className={`${formGridCls(2)} gap-3`}>
      ...
      <Campo rotulo="Matrícula anterior"><Select value={draft.matricula_anterior_id || undefined} onValueChange={(v) => set('matricula_anterior_id', v)}><SelectTrigger className={fieldCls}><SelectValue placeholder={anteriores.length ? 'Selecione...' : 'Nenhuma'} /></SelectTrigger><SelectContent>{anteriores.map((item) => <SelectItem key={item.id} value={item.id}>Matrícula {item.numero}</SelectItem>)}</SelectContent></Select></Campo>
