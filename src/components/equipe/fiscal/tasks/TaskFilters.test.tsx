...
vi.mock('@/hooks/useTaxReferenceData', () => ({ useExternalClients: () => ({ data: [] }) }));
...
function montar(filters: TaskFiltersType) {
  ...
  const view = render(
    <TaskFilters filters={filters} onFiltersChange={onFiltersChange} teamMembers={teamMembers} />,
  );
  const rerender = (next: TaskFiltersType) =>
    view.rerender(
      <TaskFilters filters={next} onFiltersChange={onFiltersChange} teamMembers={teamMembers} />,
    );
