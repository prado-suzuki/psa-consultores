...
interface ContratosTabProps {
  ...
  draftContract: {
    ...
    distribuicao_receita: Array<{ id_centro_custo: string; percentual_rateio: number }>;
  };
  setDraftContract: React.Dispatch<React.SetStateAction<ContratosTabProps["draftContract"]>>;
  ...
  editingContractData: Partial<DraftContract> | null;
  setEditingContractData: React.Dispatch<React.SetStateAction<Partial<DraftContract> | null>>;
  onStartEdit: (c: DraftContract) => void;
  onCancelEdit: () => void;
  onSaveEdit: () => void;
  onAddContract: () => void;
...
export const ContratosTab = ({
  ...
  const renderServiceSelect = (source: any[], value: string, onValueChange: (v: string) => void) => {
  ...
  const renderDistribuicaoReceita = (
    distribuicao: Array<{ id_centro_custo: string; percentual_rateio: number }>,
    onUpdate: (updated: Array<{ id_centro_custo: string; percentual_rateio: number }>) => void,
  ) => (
    <div className="border border-dashed rounded-lg p-3 mt-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <Button
          ...
          onClick={() => onUpdate([...distribuicao, { id_centro_custo: "", percentual_rateio: 0 }])}
        >
      ...
      {distribuicao.map((cc, idx) => (
        <div key={idx} className="flex items-center gap-2 mt-2">
          <Select
            value={cc.id_centro_custo || "__none__"}
            onValueChange={(v) => {
              const updated = [...distribuicao];
          ...
          <Button
            ...
            onClick={() => onUpdate(distribuicao.filter((_, i) => i !== idx))}
          >
            <X size={14} />
          </Button>
        </div>
      ...
      {distribuicao.length > 0 && (() => {
        const total = distribuicao.reduce((acc, cc) => acc + (cc.percentual_rateio || 0), 0);
        return (
          <p
            className={cn(
                    ...
                    <div className="px-4 pb-4 border-t pt-3">
                      ...
                      <div className="mt-4">
                        ...
                        {renderServiceSelect(
                          ...
                          (ec as any).id_servico || "__none__",
                          (v) => setEditingContractData({
                            ...ec,
                            id_servico: v === "__none__" ? "" : v,
                          } as any),
                        )}
                      </div>
                      {renderDistribuicaoReceita(
                        (ec as any).distribuicao_receita || [],
                        (updated) => setEditingContractData(prev => prev ? ({ ...prev, distribuicao_receita: updated } as any) : prev),
                      )}
                      {/* Observações */}
                      <div className="mt-4">
          ...
          <div className="bg-muted/50 rounded-lg border p-4">
            ...
            <div className="mt-4">
              ...
              {renderServiceSelect(
                ...
                draftContract.id_servico || "__none__",
                (v) => setDraftContract(prev => ({ ...prev, id_servico: v === "__none__" ? "" : v })),
              )}
            </div>
            ...
            <div className="flex justify-end mt-4 pt-2 border-t">
              <Button onClick={onAddContract} disabled={isAddingContract} className="gap-2">
