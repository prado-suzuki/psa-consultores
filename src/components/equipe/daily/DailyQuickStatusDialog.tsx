...
const STATUS_OPTIONS: Array<{
...
}> = [
  {
    ...
    shortLabel: 'Fazer',
    icon: Circle,
    activeClass: 'border-slate-300 bg-slate-100 text-slate-700 shadow-sm',
  },
  {
    value: 'in_progress',
    label: 'Em progresso',
    shortLabel: 'Fazendo',
    icon: Clock3,
    activeClass: 'border-teal-300 bg-teal-50 text-teal-700 shadow-sm shadow-teal-100',
  },
  {
    value: 'completed',
    label: 'Concluído',
    shortLabel: 'Concluir',
    icon: CheckCircle2,
    activeClass: 'border-teal-400 bg-teal-100 text-teal-800 shadow-sm shadow-teal-100',
  },
];
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] w-[calc(100%-1.5rem)] gap-0 overflow-hidden border-0 p-0 sm:max-w-3xl sm:rounded-2xl">
        <div className="relative overflow-hidden border-b border-teal-100 bg-gradient-to-br from-teal-50 via-white to-teal-50/70 px-5 py-5 sm:px-7">
          <div className="pointer-events-none absolute -right-12 -top-16 h-40 w-40 rounded-full bg-teal-200/40 blur-3xl" />
          <div className="pointer-events-none absolute -bottom-16 left-1/3 h-32 w-32 rounded-full bg-teal-100/60 blur-3xl" />
          <DialogHeader className="relative pr-8 text-left">
            <div className="mb-1 flex items-center gap-3">
              <motion.div
                initial={{ rotate: -12, scale: 0.8 }}
                animate={{ rotate: 0, scale: 1 }}
                className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-600 text-white shadow-lg shadow-teal-200"
              >
              ...
              <div>
                <DialogTitle className="flex items-center gap-2 text-xl text-slate-900">
                  Atualização rápida
                  <Sparkles className="h-4 w-4 text-teal-500 motion-safe:animate-pulse" />
                </DialogTitle>
            ...
            <div className="relative mt-4 flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="h-11 border-white/80 bg-white/85 pl-9 shadow-sm backdrop-blur focus-visible:ring-teal-500"
                />
              </div>
              <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
                <Badge variant="secondary" className="h-7 bg-white/80 px-2.5 text-slate-600">
                  {tasks.length} {tasks.length === 1 ? 'tarefa' : 'tarefas'}
                </Badge>
                <Badge className="h-7 bg-teal-100 px-2.5 text-teal-700 hover:bg-teal-100">
        ...
        <div className="h-[min(32rem,58vh)] min-w-0 overflow-y-auto overflow-x-hidden bg-slate-50/60">
              ...
              return (
                <section key={group.status} className="space-y-2.5 pt-3 first:pt-0">
                  <div className="flex items-center gap-2 px-1 text-xs font-semibold uppercase tracking-wider text-slate-500">
                    ...
                    <span className="rounded-full bg-slate-200/70 px-1.5 py-0.5 text-[10px] tabular-nums text-slate-600">
                      {group.tasks.length}
                    </span>
              ...
              {group.tasks.map((task, index) => {
                ...
                return (
                  <motion.article
                    ...
                    className={cn(
                      'w-full min-w-0 max-w-full overflow-hidden rounded-xl border bg-white shadow-sm transition-colors',
                      task.status === 'completed'
                        ? 'border-slate-200 bg-slate-50/70 opacity-70 shadow-none'
                        : 'border-teal-200 hover:border-teal-300',
                      isCompleting && 'border-teal-300 ring-2 ring-teal-100',
                    )}
                  >
                    <div className="flex min-w-0 flex-col gap-3 overflow-hidden p-3.5 sm:flex-row sm:items-center sm:justify-between sm:p-4">
                        ...
                        <div className="flex items-center gap-2">
                          {task.task_code && (
                            <span className={cn(
                              ...
                              task.status === 'completed' ? 'text-slate-400' : 'text-teal-600',
                            )}>
                              {task.task_code}
                            </span>
                          )}
                          {task.parent_id && <Badge variant="outline" className="h-5 px-1.5 text-[10px] text-slate-500">Subtarefa</Badge>}
                        </div>
                        <p className={cn(
                          'mt-1 block max-w-full truncate text-sm font-medium text-slate-800 sm:text-[15px]',
                          task.status === 'completed' && 'text-slate-500 line-through decoration-teal-400',
                        )}>
                          {task.title}
                        </p>
                      </div>
                      ...
                      {task.status === 'completed' ? (
                        <Badge className="h-8 shrink-0 gap-1.5 border border-slate-200 bg-slate-100 px-3 text-slate-500 shadow-none hover:bg-slate-100">
                          <CheckCircle2 className="h-4 w-4" />
                          Concluído
                        </Badge>
                      ) : (
                        <div className="grid shrink-0 grid-cols-3 gap-1 rounded-lg bg-slate-100 p-1 sm:w-[20rem]">
                          {STATUS_OPTIONS.map((option) => {
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  'flex h-9 items-center justify-center gap-1.5 rounded-md border border-transparent px-1 text-[11px] font-semibold transition-all duration-200 hover:-translate-y-0.5 hover:bg-white hover:text-slate-800 disabled:pointer-events-none sm:text-xs',
                                  active ? option.activeClass : 'text-slate-500',
                                )}
                    ...
                    <AnimatePresence>
                      {isCompleting && (
                        <motion.form
                          ...
                          className="overflow-hidden border-t border-teal-100 bg-gradient-to-r from-teal-50/90 to-teal-50/40"
                        >
                          <div className="flex flex-col gap-3 p-3.5 sm:flex-row sm:items-end sm:px-4">
                            <div className="flex-1 space-y-1.5">
                              <Label htmlFor={`actual-hours-${task.id}`} className="flex items-center gap-1.5 text-xs font-semibold text-teal-900">
                                <Clock3 className="h-3.5 w-3.5" /> Horas realizadas <span className="text-rose-500">*</span>
                              </Label>
                              <div className="flex flex-wrap items-center gap-2">
                                <Input
                                  ...
                                  placeholder="Ex.: 3,5"
                                  className={cn(
                                    'h-9 bg-white focus-visible:border-teal-300 focus-visible:ring-0 focus-visible:ring-offset-0 sm:max-w-44',
                                    hoursError && 'border-rose-400 focus-visible:border-rose-400',
                                  )}
                                />
                                <div className="flex h-9 items-center gap-1.5 rounded-md border border-teal-200 bg-white/70 px-3 text-xs text-teal-800">
                                  <TimerReset className="h-3.5 w-3.5 text-teal-600" />
...
function TaskListSkeleton() {
  return (
    <div className="space-y-2.5" aria-label="Carregando tarefas">
      {[0, 1, 2].map((item) => (
        <div key={item} className="flex h-[74px] animate-pulse items-center justify-between rounded-xl border border-slate-200 bg-white px-4">
          <div className="space-y-2"><div className="h-2.5 w-16 rounded bg-slate-200" /><div className="h-3.5 w-44 rounded bg-slate-200" /></div>
          <div className="hidden h-10 w-72 rounded-lg bg-slate-100 sm:block" />
        </div>
      ))}
    </div>
  );
