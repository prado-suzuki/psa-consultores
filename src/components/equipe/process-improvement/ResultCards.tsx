    ...
    <Card className="bg-primary/5 border-primary/20">
      <CardContent className="pt-4">
        ...
        <div className="grid grid-cols-4 gap-4 text-center">
          ...
          <div><p className="text-2xl font-bold text-success">R$ {savingsMonthly.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}</p><p className="text-xs text-muted-foreground">Economia mensal</p></div>
          <div><p className="text-2xl font-bold text-success">{savingsPercent.toFixed(0)}%</p><p className="text-xs text-muted-foreground">Redução de custo</p></div>
          <div><p className="text-2xl font-bold text-foreground">{(savedHours / 176).toFixed(2)}</p><p className="text-xs text-muted-foreground">FTE liberados</p></div>
        </div>
      </CardContent>
    </Card>
  );
...
export function RoiResultCard({ results }: { results: DomainProcessRoiResults }) {
  return (
    <Card className="bg-success/10 border-success/40">
      <CardContent className="pt-4">
        <h4 className="font-semibold text-foreground mb-3 flex items-center gap-2"><CheckCircle2 className="h-5 w-5" />ROI Calculado</h4>
        <div className="grid grid-cols-4 gap-4 text-center">
          ...
          <div><p className="text-2xl font-bold text-success">R$ {results.cost_saved_monthly?.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}</p><p className="text-xs text-muted-foreground">Economia mensal</p></div>
          ...
          <div><p className="text-2xl font-bold text-foreground">{results.payback_months?.toFixed(1)} meses</p><p className="text-xs text-muted-foreground">Payback</p></div>
        </div>
      </CardContent>
    </Card>
  );
