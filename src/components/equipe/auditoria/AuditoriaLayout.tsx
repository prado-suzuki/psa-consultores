import { ArrowLeft, ChevronLeft, ChevronRight, ClipboardList, FolderKanban, Home, LayoutDashboard, ListChecks, Menu, MessageSquare, MessagesSquare, ClipboardCheck } from 'lucide-react';
...
const ITENS_TOPO: ItemDeMenu[] = [
  { id: 'inicio', label: 'Início', icon: Home, path: '/equipe/auditoria/inicio', exato: true },
  { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, path: '/equipe/auditoria/dashboard' },
];

const GRUPO_PROJETOS: readonly ItemDoGrupo[] = [
  { path: '/equipe/auditoria/projetos/clientes', label: 'Clientes', icon: ClipboardList },
  { path: '/equipe/auditoria/projetos/cadastro', label: 'Projetos e tarefas', icon: FolderKanban },
  { path: '/equipe/auditoria/projetos/controle', label: 'Controle de Projetos', icon: ListChecks },
  { path: '/equipe/auditoria/projetos/feed', label: 'Feed', icon: MessagesSquare },
];
...
const MENU_WORK: ItemDeMenu[] = [
  { id: 'work', label: 'Início', icon: Home, path: '/equipe/auditoria/work', exato: true },
];

export type AuditoriaLayoutProps = {
...
export const AuditoriaLayout = (props: AuditoriaLayoutProps) => {
  const { children, headerActions, rolagemNoConteudo = false } = props;
  const { title, subtitle } = resolverCabecalho(props, 'auditoria');
  ...
  const isWork = pathname.startsWith('/equipe/auditoria/work');
  const menu = isWork ? MENU_WORK : ITENS_TOPO;
  const area = isWork ? AREAS.auditoriaWork : AREAS.auditoriaProjects;
  const projetosAtivo = pathname.startsWith('/equipe/auditoria/projetos');
    ...
    <div className={cn('flex w-full', rolagemNoConteudo ? 'h-screen overflow-hidden' : 'min-h-screen')}>
      ...
      >
          ...
          {!isWork && (
            <ButtonTooltip text={trilho ? 'Chamados' : undefined}>
              <button
                ...
                onClick={() => navigate(linkEspelhado('/equipe/chamados', 'auditoria'))}
              >
        ...
        <div className="mt-auto p-4 border-t border-border/60 space-y-2">
          <SidebarCartaoUsuario area="auditoria" collapsed={trilho} />
          ...
          <ButtonTooltip text={trilho ? 'Trocar ambiente' : undefined}>
            <Button
              ...
              onClick={() => navigate('/equipe/auditoria')}
            >
