  ...
  return (
    <>
      ...
      {snapshots.length > 0 && <div className="roi-callout is-info"><span className="roi-callout-icon"><Icon name="info" size={16} /></span><div className="roi-callout-body" style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <select id="roi-snapshot-selector" className="roi-snapshot-select" value={snapshotSelecionado} onChange={event => onSelecionarSnapshot(event.target.value)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid hsl(var(--slate-300))', background: '#fff', fontSize: '0.85rem', minWidth: 280 }}>
        ...
        {historico && <span style={{ fontSize: '0.78rem', color: 'hsl(var(--slate-600))' }}>Você está vendo dados <strong>estáticos</strong> de uma mensuração anterior. Voltar para "Ao vivo" para salvar nova mensuração.</span>}
      </div></div>}
      ...
      <div className="roi-subhead">Indicadores consolidados{historico && <span style={{ marginLeft: 8, fontSize: '0.78rem', color: 'hsl(var(--primary))', fontWeight: 600 }}>· Mensuração de {new Date(snapAtivo.snapshot_at).toLocaleString('pt-BR')}</span>}<span className="roi-subhead-rule" /></div>
