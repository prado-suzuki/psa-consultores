  ...
  return <>
    <div className="mapear-topbar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <ButtonTooltip text="Voltar à listagem de processos">
        <button className="mapear-voltar" onClick={onVoltar}><ArrowLeft size={16} strokeWidth={2.2} /><span>Processos</span></button>
      </ButtonTooltip>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><TourTrigger dataTour="help" /><ButtonTooltip text="Editar este processo">
        <button className="mapear-voltar" onClick={onEditarProcesso}><Settings2 size={15} strokeWidth={2.2} /><span>Editar processo</span></button>
      </ButtonTooltip></div>
    </div>
    ...
    <div className="mapear-downloads" role="group" aria-label="Exportar documentos do processo">
      ...
      <ButtonTooltip text="Baixar SOP — Como Era (cenário atual)">
        <button className="mapear-dl-btn" onClick={() => onSop('era')}><FileText size={15} strokeWidth={2.1} /><span>SOP (antes)</span></button>
      </ButtonTooltip>
      <ButtonTooltip text="Baixar SOP — Como Ficou (cenário projetado)">
        <button className="mapear-dl-btn" onClick={() => onSop('ficou')}><FileText size={15} strokeWidth={2.1} /><span>SOP (como ficou)</span></button>
      </ButtonTooltip>
      <ButtonTooltip text="Baixar SOP comparativo Como Era × Como Ficou, lado a lado, com ganhos por etapa e ROI consolidado">
        <button className="mapear-dl-btn" onClick={onComparativo}><GitCompare size={15} strokeWidth={2.1} /><span>SOP (comparativo)</span></button>
      </ButtonTooltip>
      <ButtonTooltip text="Baixar SOP em Markdown — Como Era (mesmo conteúdo do PDF; ideal para refinar o mapeamento)">
        <button className="mapear-dl-btn" onClick={() => onMarkdown('era')}><FileCode2 size={15} strokeWidth={2.1} /><span>SOP MD (antes)</span></button>
      </ButtonTooltip>
      <ButtonTooltip text="Baixar SOP em Markdown — Como Ficou (cenário projetado)">
        <button className="mapear-dl-btn" onClick={() => onMarkdown('ficou')}><FileCode2 size={15} strokeWidth={2.1} /><span>SOP MD (como ficou)</span></button>
      </ButtonTooltip>
      <ButtonTooltip text="Baixar SOP comparativo em Markdown (mesmo conteúdo do PDF comparativo)">
        <button className="mapear-dl-btn" onClick={onMarkdownComparativo}><FileCode2 size={15} strokeWidth={2.1} /><span>SOP MD (comparativo)</span></button>
      </ButtonTooltip>
      <ButtonTooltip text="Visualizar e baixar o diagrama de ligações do processo — Como Era (cenário atual)">
        <button className="mapear-dl-btn" onClick={() => onDiagrama('era')}><Network size={15} strokeWidth={2.1} /><span>Diagrama (antes)</span></button>
      </ButtonTooltip>
      {temFicou && <ButtonTooltip text="Visualizar e baixar o diagrama de ligações do processo — Como Ficou (cenário projetado)">
        <button className="mapear-dl-btn" onClick={() => onDiagrama('ficou')}><Network size={15} strokeWidth={2.1} /><span>Diagrama (como ficou)</span></button>
      </ButtonTooltip>}
    </div>
  </>;
}
