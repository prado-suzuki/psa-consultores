...
export function EFDExportDialog({ arquivo, blocosDisponiveis, idContribuinte, disabled, tipo = 'contribuicoes', profileType, externalOpen, onExternalOpenChange, hideTrigger = false }: EFDExportDialogProps) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    {!hideTrigger && <TooltipProvider><Tooltip><TooltipTrigger asChild><DialogTrigger asChild><Button variant="outline" size="icon" disabled={disabled || !allRecords.length} className="h-9 w-9 text-accent-d hover:text-accent-d bg-primary/5 border-primary/30"><FileDown className="h-4 w-4" /></Button></DialogTrigger></TooltipTrigger><TooltipContent><p>Exportar Excel</p></TooltipContent></Tooltip></TooltipProvider>}
    <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col p-0 overflow-hidden">
      <DialogHeader className="p-6 pb-4 border-b border-border bg-muted/50 flex-shrink-0">
        <DialogTitle className="text-xl flex items-center gap-2"><FileDown className="h-6 w-6 text-accent-d" />Exportar para Excel</DialogTitle>
