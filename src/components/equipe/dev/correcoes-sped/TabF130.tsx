...
export default function TabF130({ data, isLoading, error, hasQueried, searchText, empresaCnpj, periodo }: TabF130Props) {
  ...
  const [draft, setDraft] = useState<F130Draft | null>(null);
  ...
  useEffect(() => {
    ...
    setRows(data.map(d => {
      if (locallyEditedIds.current.has(d.F130.uuid)) {
        const local = rows.find(r => r.F130.uuid === d.F130.uuid);
        return local ?? d;
      }
      return d;
    }));
  ...
  const handleStartEdit = (item: F130Item) => { setEditingId(item.F130.uuid); setDraft(toDraft(item)); };
  const handleCancelEdit = () => { setEditingId(null); setDraft(null); };
  const handleDraftChange = (field: EditableF130Field, value: string) => { setDraft((c) => c ? { ...c, [field]: value } : c); };

  const handleSave = async (item: F130Item) => {
    if (!user || !draft || editingId !== item.F130.uuid) return;
    ...
    try {
      ...
      if (camposAlterados.length === 0) {
        ...
        setRows((c) => c.map((r) => r.F130.uuid === item.F130.uuid ? { ...r, F130: { ...item._originalSnapshot } } : r));
      ...
      setRows((c) => c.map((r) => r.F130.uuid === item.F130.uuid ? { ...r, F130: { ...r.F130, ...nextSnapshot } as F130Reg } : r));
      locallyEditedIds.current.add(item.F130.uuid);
      handleCancelEdit();
      toast.success('Correção do F130 salva.');
  ...
  const renderEditableCell = (
    ...
    if (editingId !== item.F130.uuid || !draft) {
              ...
              <Table>
                ...
                <TableBody>
                  {paged.map((item, idx) => {
                    ...
                    return (
                      <TableRow key={`f130-${item.F130.uuid}-${idx}`} className={editingId === item.F130.uuid ? 'bg-accent/30' : 'group'}>
                        ...
                        <TableCell className="py-1.5 sticky right-0 bg-background z-10 w-[90px] min-w-[90px] max-w-[90px] border-l border-slate-200 dark:border-slate-700 shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                          <div className="flex flex-col items-center justify-center gap-1">
                            {editingId === item.F130.uuid ? (
                              <>
                                <Button type="button" size="icon" variant="ghost" className="h-8 w-8" onClick={() => handleSave(item)} disabled={savingId === item.F130.uuid}>{savingId === item.F130.uuid ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4 text-emerald-600" />}</Button>
                                <Button type="button" size="icon" variant="ghost" className="h-8 w-8" onClick={handleCancelEdit} disabled={savingId === item.F130.uuid}><X className="h-4 w-4 text-muted-foreground" /></Button>
                              </>
                            ) : (
                            ...
                            {linhaCorrigida && editingId !== item.F130.uuid && <Badge variant="outline" className="text-[10px]">Corrigido</Badge>}
                          </div>
                        </TableCell>
                      </TableRow>
                    );
