...
type EditableF120Field = 'IDENT_BEM_IMOB' | 'IND_UTIL_BEM_IMOB' | 'NAT_BC_CRED' | 'VL_OPER_DEP' | 'CST_PIS' | 'ALIQ_PIS' | 'VL_PIS' | 'CST_COFINS' | 'ALIQ_COFINS' | 'VL_COFINS' | 'COD_CTA';
type F120Draft = Record<EditableF120Field, string>;

const editableFields: EditableF120Field[] = ['IDENT_BEM_IMOB', 'IND_UTIL_BEM_IMOB', 'NAT_BC_CRED', 'VL_OPER_DEP', 'CST_PIS', 'ALIQ_PIS', 'VL_PIS', 'CST_COFINS', 'ALIQ_COFINS', 'VL_COFINS', 'COD_CTA'];
const numericFields = new Set<EditableF120Field>(['VL_OPER_DEP', 'CST_PIS', 'ALIQ_PIS', 'VL_PIS', 'CST_COFINS', 'ALIQ_COFINS', 'VL_COFINS']);
const codeFields = new Set<EditableF120Field>(['IDENT_BEM_IMOB', 'IND_UTIL_BEM_IMOB', 'NAT_BC_CRED']);
...
type CodeField120 = 'IDENT_BEM_IMOB' | 'IND_UTIL_BEM_IMOB' | 'NAT_BC_CRED';
const DESC_KEY_BY_CODE_FIELD: Record<CodeField120, 'DESC_IDENT_BEM_IMOB' | 'DESC_IND_UTIL_BEM_IMOB' | 'DESC_NAT_BC_CRED'> = {
  IDENT_BEM_IMOB: 'DESC_IDENT_BEM_IMOB',
  IND_UTIL_BEM_IMOB: 'DESC_IND_UTIL_BEM_IMOB',
  NAT_BC_CRED: 'DESC_NAT_BC_CRED',
};

const OPTIONS_BY_CODE_FIELD: Record<CodeField120, { code: string; description: string }[]> = {
...
export default function TabF120({ data, isLoading, error, hasQueried, searchText, empresaCnpj, periodo, contribuinteId, onEnviar, onExportar, isSending, isExporting, pendingCount, idArquivos }: TabF120Props) {
    ...
    try {
      ...
      for (const item of data) {
        ...
        const correcaoAtiva = await buscarCorrecaoSpedAtiva({
          registroTipo: 'F120',
          registroOriginalId: item.F120.uuid,
        });
  ...
  const renderCodeCell = (item: F120Item, field: CodeField120) => {
    const draft = drafts[item.F120.uuid];
              ...
              <CommandGroup>
                {options.map((option) => (
                  <CommandItem
                    ...
                    onSelect={() => handleDraftChange(item.F120.uuid, field, option.code)}
              ...
              <Table containerRef={scrollRef}>
                <TableHeader>
                  ...
                  <TableRow>
                    ...
                    <TableHead className="text-[11px] min-w-[200px] whitespace-normal break-words"><span className="flex items-center gap-1">{renderColumnLabel('Bem Imobilizado', SPED_TOOLTIPS.f120Bem)}<ColumnFilterDropdown columnKey="DESC_IDENT_BEM_IMOB" uniqueValues={cascadingUniqueValues['DESC_IDENT_BEM_IMOB'] ?? []} activeSort={sortConfig} activeFilter={columnFilters['DESC_IDENT_BEM_IMOB'] ?? null} onSort={handleSort} onFilter={handleFilter} /></span></TableHead>
                    <TableHead className="text-[11px] min-w-[140px] whitespace-normal break-words"><span className="flex items-center gap-1">{renderColumnLabel('Utilização', SPED_TOOLTIPS.f120Utilizacao)}<ColumnFilterDropdown columnKey="DESC_IND_UTIL_BEM_IMOB" uniqueValues={cascadingUniqueValues['DESC_IND_UTIL_BEM_IMOB'] ?? []} activeSort={sortConfig} activeFilter={columnFilters['DESC_IND_UTIL_BEM_IMOB'] ?? null} onSort={handleSort} onFilter={handleFilter} /></span></TableHead>
                    ...
                    <TableHead className="text-[11px] text-right min-w-[120px]">{renderColumnLabel('Depreciação', SPED_TOOLTIPS.f120Depreciacao)}</TableHead>
                ...
                <TableBody>
                  {paged.map((item, idx) => {
                    ...
                    return (
                      <TableRow key={`f120-${item.F120.uuid}-${idx}`} className={isEditMode ? (selection.selectedIds.has(item.F120.uuid) ?'bg-primary/15':'bg-primary/5') :'group'}>
                        {isEditMode && (
