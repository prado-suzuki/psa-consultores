...
import type { F120Item } from '@/types/correcoesSped';
...
export default function TabF120({ data, isLoading, error, hasQueried, searchText }: TabF120Props) {
  ...
  if (isLoading) {
    return (
      <Card><CardContent className="p-8 flex justify-center">
        <div className="animate-pulse text-sm text-muted-foreground">Carregando dados F120...</div>
      </CardContent></Card>
    );
  }
      ...
      <CardContent className="p-0">
        {filtered.length === 0 ? (
          <div className="p-8 text-center text-sm text-muted-foreground">
            Nenhum item F120 encontrado para os filtros selecionados.
          </div>
        ) : (
          <>
            ...
            <div className="overflow-auto">
              <Table>
                  ...
                  <TableRow className="border-b-0">
                    <TableHead colSpan={4} className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground/70 pb-0 pt-2 bg-muted/40">
                      <span className="flex items-center gap-1">
                        Dados do Bem
                        <Tooltip><TooltipTrigger asChild><Info className="h-3 w-3 cursor-help text-muted-foreground/70" /></TooltipTrigger><TooltipContent side="top" className="max-w-xs text-xs">Registro F120 — Bens do ativo imobilizado com depreciação/amortização geradores de crédito.</TooltipContent></Tooltip>
                      </span>
                    </TableHead>
                ...
                <TableBody>
                  {paged.map((item, idx) => (
                    <TableRow key={`f120-${item.F120.uuid}-${idx}`} className="group">
                      ...
                      <TableCell className="text-xs text-right py-1.5 font-mono tabular-nums">{formatCurrency(item.F120.VL_OPER_DEP)}</TableCell>
                      {/* Tax zone */}
                      <TableCell className="text-xs text-center py-1.5 font-mono border-l-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-800/10">{item.F120.CST_PIS}</TableCell>
                      <TableCell className="text-xs text-right py-1.5 font-mono tabular-nums bg-slate-50/30 dark:bg-slate-800/10">{safeFixed(item.F120.ALIQ_PIS)}</TableCell>
                      <TableCell className="text-xs text-right py-1.5 font-mono tabular-nums bg-slate-50/30 dark:bg-slate-800/10">{formatCurrency(item.F120.VL_PIS)}</TableCell>
                      <TableCell className="text-xs text-center py-1.5 font-mono bg-slate-50/30 dark:bg-slate-800/10">{item.F120.CST_COFINS}</TableCell>
                      <TableCell className="text-xs text-right py-1.5 font-mono tabular-nums bg-slate-50/30 dark:bg-slate-800/10">{safeFixed(item.F120.ALIQ_COFINS)}</TableCell>
                      <TableCell className="text-xs text-right py-1.5 font-mono tabular-nums bg-slate-50/30 dark:bg-slate-800/10">{formatCurrency(item.F120.VL_COFINS)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
