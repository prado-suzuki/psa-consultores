              ...
              <Table containerRef={scrollRef}>
                ...
                <TableBody>
                  {paged.map((item, idx) => {
                    ...
                    return (
                      <TableRow key={`f120-${item.F120.uuid}-${idx}`} className={isEditMode ? (selection.selectedIds.has(item.F120.uuid) ? 'bg-primary/15 dark:bg-primary/20' : 'bg-primary/5 dark:bg-primary/5') : 'group'}>
                        {isEditMode && (
