              ...
              <Table containerRef={scrollRef}>
                <TableHeader>
                  <TableRow className="border-b-0">
                    ...
                    <TableHead colSpan={7} className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground/70 pb-0 pt-2 border-l-2 border-dashed border-border bg-muted/60">{renderColumnLabel('Impostos', SPED_TOOLTIPS.impostos)}</TableHead>
                  ...
                  <TableRow>
                    ...
                    <TableHead className="text-[11px] min-w-[80px]"><span className="flex items-center gap-1">{renderColumnLabel('Data', SPED_TOOLTIPS.f100Data)}<ColumnFilterDropdown columnKey="DT_OPER" uniqueValues={cascadingUniqueValues['DT_OPER'] ?? []} activeSort={sortConfig} activeFilter={columnFilters['DT_OPER'] ?? null} onSort={handleSort} onFilter={handleFilter} /></span></TableHead>
                    <TableHead className="text-[11px] min-w-[180px]"><span className="flex items-center gap-1">{renderColumnLabel('Nome', SPED_TOOLTIPS.f100Nome)}<ColumnFilterDropdown columnKey="NOME" uniqueValues={cascadingUniqueValues['NOME'] ?? []} activeSort={sortConfig} activeFilter={columnFilters['NOME'] ?? null} onSort={handleSort} onFilter={handleFilter} /></span></TableHead>
                    <TableHead className="text-[11px] min-w-[120px]"><span className="flex items-center gap-1">{renderColumnLabel('CPF/CNPJ', SPED_TOOLTIPS.f100CpfCnpj)}<ColumnFilterDropdown columnKey="CPF_CNPJ" uniqueValues={cascadingUniqueValues['CPF_CNPJ'] ?? []} activeSort={sortConfig} activeFilter={columnFilters['CPF_CNPJ'] ?? null} onSort={handleSort} onFilter={handleFilter} /></span></TableHead>
                    <TableHead className="text-[11px] min-w-[60px]"><span className="flex items-center gap-1">{renderColumnLabel('Tipo', SPED_TOOLTIPS.f100Tipo)}<ColumnFilterDropdown columnKey="TIPO_PESSOA" uniqueValues={cascadingUniqueValues['TIPO_PESSOA'] ?? []} activeSort={sortConfig} activeFilter={columnFilters['TIPO_PESSOA'] ?? null} onSort={handleSort} onFilter={handleFilter} /></span></TableHead>
                    <TableHead className="text-[11px] min-w-[80px]"><span className="flex items-center gap-1">{renderColumnLabel('Simples', SPED_TOOLTIPS.f100Simples)}<ColumnFilterDropdown columnKey="SIMPLES_LABEL" uniqueValues={cascadingUniqueValues['SIMPLES_LABEL'] ?? []} activeSort={sortConfig} activeFilter={columnFilters['SIMPLES_LABEL'] ?? null} onSort={handleSort} onFilter={handleFilter} /></span></TableHead>
                    <TableHead className="text-[11px] text-right min-w-[110px]">{renderColumnLabel('Valor', SPED_TOOLTIPS.f100Valor)}</TableHead>
                ...
                <TableBody>
                  {paged.map((item, idx) => {
                    ...
                    return (
                      <TableRow key={`f100-${item.F100.uuid}-${idx}`} className={isEditMode ? (selection.selectedIds.has(item.F100.uuid) ?'bg-primary/15':'bg-primary/5') :'group'}>
                        {isEditMode && (
                        ...
                        <TableCell className="text-xs text-center py-1.5 font-mono border-l-2 border-dashed border-border bg-muted/30">{renderEditableCell(item,'CST_PIS','h-8 text-xs text-center font-mono')}</TableCell>
                        <TableCell className="text-xs text-right py-1.5 font-mono tabular-nums bg-muted/30">{renderEditableCell(item,'ALIQ_PIS','h-8 text-xs text-right font-mono', { isPercentage: true })}</TableCell>
                        <TableCell className="text-xs text-right py-1.5 font-mono tabular-nums bg-muted/30">{renderEditableCell(item,'VL_PIS','h-8 text-xs text-right font-mono', { isCurrency: true })}</TableCell>
                        <TableCell className="text-xs text-center py-1.5 font-mono bg-muted/30">{renderEditableCell(item,'CST_COFINS','h-8 text-xs text-center font-mono')}</TableCell>
