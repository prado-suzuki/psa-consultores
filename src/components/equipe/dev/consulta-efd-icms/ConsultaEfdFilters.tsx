...
export function ConsultaEfdFilters(props: Props) {
  return <Card className="mb-6 shadow-sm"><CardHeader className="pb-4"><CardTitle className="text-lg flex items-center gap-2 text-primary"><Filter className="h-5 w-5"/><span className="uppercase text-sm tracking-wider font-bold text-slate-800">Filtros de Busca</span></CardTitle></CardHeader><CardContent className="space-y-4">
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
      <div className="md:col-span-3"><FieldLabel tip="Filtra as EFD ICMS por cliente ou grupo.">Cliente</FieldLabel><Select value={props.cliente} onValueChange={props.onCliente}><SelectTrigger className="h-11 bg-white"><SelectValue placeholder={props.loadingClientes ?'Carregando...':'Selecione o cliente'} /></SelectTrigger><SelectContent className="bg-background border z-50">{props.clientes?.map(item => <SelectItem key={item.id} value={item.id}>{item.nome}</SelectItem>)}</SelectContent></Select></div>
      <div className="md:col-span-5"><FieldLabel tip="CNPJ/CPF vinculado ao cliente. Obrigatório para a busca.">Contribuinte</FieldLabel><Select value={props.contribuinte} onValueChange={props.onContribuinte}><SelectTrigger className="h-11 bg-white"><SelectValue placeholder={props.loadingContribuintes ?'Carregando...':'Selecione o contribuinte'} /></SelectTrigger><SelectContent className="bg-background border z-50">{props.contribuintes?.map(item => <SelectItem key={item.id} value={item.id}>{item.nome_razao_social} {item.cpf_cnpj ?`(${formatCnpj(item.cpf_cnpj)})`:''}</SelectItem>)}</SelectContent></Select></div>
