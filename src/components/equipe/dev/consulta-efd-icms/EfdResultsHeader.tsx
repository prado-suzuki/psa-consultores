...
export function EfdResultsHeader(props: Props) {
  ...
  return <div className="px-6 py-4 bg-muted border-b"><div className="flex items-center justify-between"><div className="flex items-center gap-4"><Building2 className="h-5 w-5 text-primary"/><div className="flex flex-col">
    ...
    <span className="text-xs text-muted-foreground mt-0.5">CNPJ: {formatCnpj(selectedFilial?.cnpjCompleto || props.cnpj)}</span></div><Button variant="ghost" size="icon" className="h-8 w-8" onClick={props.onRefresh} disabled={props.loading}>{props.loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}</Button></div>
