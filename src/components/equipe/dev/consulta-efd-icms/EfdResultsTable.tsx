...
export function EfdResultsTable(props: Props) {
  ...
  return <div className={cn('overflow-x-auto','[&::-webkit-scrollbar]:h-3','[&::-webkit-scrollbar-thumb]:bg-muted-foreground/40','[&::-webkit-scrollbar-thumb]:rounded-full')}><table className="w-full text-left border-collapse"><thead><tr className="bg-muted border-b">
    ...
    <td className="px-6 py-4"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center text-primary"><FileSpreadsheet className="h-5 w-5"/></div><div><p className="font-bold text-sm text-foreground">{arquivo.NOME}</p><p className="text-[10px] text-muted-foreground uppercase font-mono">ID: {arquivo.ID_ARQUIVO}</p></div></div></td>
