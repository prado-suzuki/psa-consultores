...
export function EfdResultsTable(props: Props) {
  ...
  return <div className={cn('overflow-x-auto', '[&::-webkit-scrollbar]:h-3', '[&::-webkit-scrollbar-thumb]:bg-slate-400', '[&::-webkit-scrollbar-thumb]:rounded-full')}><table className="w-full text-left border-collapse"><thead><tr className="bg-slate-100 dark:bg-slate-800/80 border-b">
  ...
  </tr></thead><tbody className="divide-y divide-slate-200 dark:divide-slate-700">{props.arquivos.map(arquivo => <tr key={arquivo.ID_ARQUIVO} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
    ...
    <td className="px-6 py-4"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center text-primary"><FileSpreadsheet className="h-5 w-5" /></div><div><p className="font-bold text-sm text-slate-800 dark:text-white">{arquivo.NOME}</p><p className="text-[10px] text-slate-400 uppercase font-mono">ID: {arquivo.ID_ARQUIVO}</p></div></div></td>
...
function Empty({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return <div className="flex flex-col items-center justify-center text-center p-8 h-full min-h-[300px]"><div className="mb-4">{icon}</div><h3 className="text-xl font-bold text-slate-700 dark:text-slate-300">{title}</h3><p className="text-base text-slate-500 max-w-xs mt-2">{text}</p></div>;
}
