  ...
  return <Card>
    ...
    <CardContent className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        <div className="md:col-span-3"><Label text="Cliente"required tooltip={XML_TOOLTIPS.cliente} /><BuscaDeDominio options={opcoesCliente} value={values.cliente} onChange={set.cliente} loading={props.loadingClientes} placeholder="Selecione um cliente" searchPlaceholder="Digite o nome ou o CNPJ…" emptyText="Nenhum cliente encontrado." /></div>
        <div className="md:col-span-4"><Label text="Contribuinte"required tooltip={XML_TOOLTIPS.contribuinte} />{props.errorContribuintes ? <div className="text-destructive text-sm p-3 border border-destructive/50 rounded-md bg-destructive/10">{props.errorContribuintes.message}<Button variant="link"className="text-destructive p-0 h-auto ml-2"onClick={() => navigate("/equipe")}>Fazer login novamente</Button></div> : <BuscaDeDominio options={opcoesContribuinte} value={values.contribuinte} onChange={set.contribuinte} loading={props.loadingContribuintes} placeholder="Selecione um contribuinte" searchPlaceholder="Digite o nome ou o CNPJ…" emptyText="Nenhum contribuinte encontrado." />}</div>
 ...
 * `md:col-span-3`.
 */
function BuscaDeDominio({ options, value, onChange, loading, placeholder, searchPlaceholder, emptyText }: {
  ...
  return <SingleSelectCombobox options={options} value={value || null} onChange={(escolhido) => onChange(escolhido ?? "")} disabled={loading} placeholder={loading ? "Carregando..." : placeholder} searchPlaceholder={searchPlaceholder} emptyText={emptyText} className="w-full min-w-0 h-11" />;
}
