...
import {
  FATOS_CLIENTES,
  FATOS_POR_UF_PRODUTO,
  TOTAL_CLIENTES_DISTINTOS,
  type FatoUfProduto,
  type NaturezaDestino,
} from "./AbaPorEstado.mock";
...
export function AbaPorEstado({ filtros }: AbaPorEstadoProps) {
  ...
  const fatosFiltrados = useMemo(() => {
    return FATOS_POR_UF_PRODUTO.filter((f) => {
  ...
  }, [filtros.ufs, filtros.anexos]);

  const clientesFiltrados = useMemo(() => {
    return FATOS_CLIENTES.filter((c) => {
  ...
  // (fonte de verdade = totais.faturamento dos fatos UF×produto, não a soma dos
  // clientes mockados que é incompleta — apenas top 12 modelados de 187 reais).
  ...
  return (
    <div className="space-y-6">
      <Alert className="bg-blue-50 border-blue-200">
        ...
        <AlertDescription className="text-blue-900 text-xs space-y-1">
          <p>
            <strong>Visualização com dados mockados</strong> (extraídos do CSV real de saídas 2025) —
            endpoint <code className="font-mono text-[11px] bg-blue-100 px-1 rounded">/api/v1/calculadora_ibs_cbs/por_uf</code>{" "}
            pendente.
          </p>
          <p className="text-[11px] text-blue-700">
            <strong>Filtros UF e Anexo funcionam localmente</strong>; o filtro de Período é ignorado nesta aba
            até que o backend esteja disponível.
          </p>
        </AlertDescription>
      </Alert>
            ...
            <>
              ...
              <p className="text-slate-400">
                {filtrosAtivos
                  ...
                  : `${fmtInt(TOTAL_CLIENTES_DISTINTOS)} clientes distintos`}
              </p>
            </>
          }
      ...
      <UfDrillDown uf={ufDrillDown} onClose={() => setUfDrillDown(null)} filtros={filtros} />
    </div>
  );
}
