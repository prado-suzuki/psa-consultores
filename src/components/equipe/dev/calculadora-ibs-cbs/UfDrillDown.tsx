...
export function UfDrillDown({ uf, onClose, filtros }: UfDrillDownProps) {
  const open = uf !== null;
  const anexosFiltro = filtros?.anexos ?? [];

  const dados = useMemo(() => {
    if (!uf) return null;
    const produtos = FATOS_POR_UF_PRODUTO.filter((f) => {
      if (f.uf !== uf) return false;
      if (anexosFiltro.length > 0 && !anexosFiltro.includes(f.anexo)) return false;
      return true;
    });
    const clientes = FATOS_CLIENTES.filter((c) => {
    ...
    return {
      ...
      clientes: clientes.sort((a, b) => b.faturamento - a.faturamento),
      faturamento,
      tribAntes,
      tribDepois,
      qtdNFs,
        ...
        {uf && dados && (
          <>
            ...
            <p className="text-[10px] text-slate-400 mt-4 italic">
              Dados do mock (CSV Barralcool 2025). Top clientes limitado ao ranking global das 12 maiores
              razões sociais.
            </p>
          </>
        )}
