    ...
    <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
      <Card className="border-border xl:col-span-2">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold text-foreground flex items-center gap-1.5">Mapa de exposição por UF<HelpHint><strong>Intensidade da cor</strong> proporcional ao faturamento. <strong>Cor do badge</strong> indica a natureza (Interno/Inter./Exp.). <strong>Alíq efet.</strong> é a alíquota efetiva ponderada de IBS/CBS sobre o faturamento da UF. <strong>Δ pp</strong> compara a carga antes vs depois. <strong>Clique em qualquer UF</strong> para abrir o drill-down completo (produtos + clientes).</HelpHint></CardTitle>
          <p className="text-xs text-muted-foreground">Clique em uma UF para ver detalhes · intensidade = faturamento · cor = natureza</p>
        </CardHeader>
        <CardContent><div className="grid grid-cols-2 gap-3">{ufs.map((uf) => {
          const cor = CORES_NATUREZA[uf.natureza];
          return <button key={uf.uf} type="button" onClick={() => onSelecionarUf(uf.uf)} className="group relative rounded-lg border border-border overflow-hidden bg-card text-left hover:border-muted-foreground/50 hover:shadow-md transition-all cursor-pointer" title={`Ver detalhamento de ${uf.uf}`}>
