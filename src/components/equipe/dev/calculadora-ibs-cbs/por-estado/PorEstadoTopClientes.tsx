...
export function PorEstadoTopClientes({ clientes, quantidadeClientes, faturamentoTotal, pctTop3Clientes }: PorEstadoTopClientesProps) {
  return (
    <Card className="border-border">
      <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold text-foreground flex items-center gap-1.5"><Users className="h-4 w-4 text-primary" />Top 12 Clientes — Quem está demandando<HelpHint>Clientes agrupados pelo <strong>nome do destinatário (nome_dest)</strong>, consolidando filiais (CNPJs distintos com mesmo nome). Universo do filtro atual: <strong>{fmtInt(quantidadeClientes)} clientes</strong>. Top 3 representam <strong>{pctTop3Clientes.toFixed(1)}%</strong> do faturamento do filtro — métrica clássica de risco de concentração comercial.</HelpHint></CardTitle><p className="text-xs text-muted-foreground">Top 3 concentram <strong>{pctTop3Clientes.toFixed(1)}%</strong> do faturamento — monitorar risco de concentração</p></CardHeader>
      <CardContent className="p-0">{clientes.length === 0 ? <p className="text-sm text-muted-foreground py-12 text-center">Sem clientes para os filtros atuais</p> : <div className="overflow-x-auto"><Table>
