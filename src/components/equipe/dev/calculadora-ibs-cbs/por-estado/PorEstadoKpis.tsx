  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <KpiCard
        ...
        sub={<><p className="text-slate-400">Antes: <strong className="text-slate-600">{totais.cargaAntesPct.toFixed(2)}%</strong> · Depois: <strong className="text-slate-600">{totais.cargaDepoisPct.toFixed(2)}%</strong></p><div className={`mt-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-semibold ${totais.deltaPp < 0 ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{totais.deltaPp < 0 ? <TrendingDown className="h-3 w-3" /> : <TrendingUp className="h-3 w-3" />}{totais.deltaPp < 0 ? "Reforma alivia carga" : "Reforma aumenta carga"}</div></>}
