  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <KpiCard
        ...
        sub={<><p className={`font-semibold ${pctTop3Ufs > 80 ? "text-destructive" : pctTop3Ufs > 60 ? "text-status-alerta" : "text-success"}`}>{pctTop3Ufs > 80 ? "Alta concentração geográfica" : pctTop3Ufs > 60 ? "Concentração moderada" : "Distribuição saudável"}</p><p className="text-muted-foreground">{ufs.slice(0, 3).map((uf) => uf.uf).join(" + ")}</p></>}
