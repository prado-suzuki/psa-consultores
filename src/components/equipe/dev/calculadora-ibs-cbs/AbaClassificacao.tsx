...
import { IbsCbsAuditModal } from "@/components/equipe/dev/IbsCbsAuditModal";
...
import {
  IbsCbsGroupedItem,
  IbsCbsApiGroupedResponse,
  IbsCbsApiGroupedItem,
  IbsCbsClassificacoesBuscarResponse,
  IbsCbsSyncPayload,
  IbsCbsSyncDecisao,
...
export function AbaClassificacao() {
  ...
  // Estado do modal
  const [selectedGroup, setSelectedGroup] = useState<IbsCbsGroupedItem | null>(null);
  ...
  useEffect(() => {
    const loadLastSession = async () => {
      ...
      try {
        // Buscar apenas sessões EM_ANDAMENTO
  ...
  // Query: Buscar itens agrupados do período
  const {
    data: apiGroupedData,
    ...
    queryFn: async () => {
      ...
      const data: IbsCbsApiGroupedResponse = await response.json();
      return {
        items: data.items,
        total: data.total,
  ...
  const { data: classificacoes, isLoading: isLoadingClassificacoes } = useQuery({
    queryKey: ["ibscbs-classificacoes", groupedItemsFromApi.map((i) => `${i.cod_produto}|${i.cod_ncm}`)],
    queryFn: async () => {
      ...
      return response.json() as Promise<IbsCbsClassificacoesBuscarResponse>;
    },
    enabled: groupedItemsFromApi.length > 0,
  });

  // Itens agrupados com status
  const groupedItems = useMemo(() => {
    return groupedItemsFromApi.map((group) => {
      ...
      const status: "validado" | "pendente" = isLocallyDecided ? "validado" : group.status;
  ...
  const handleGroupClick = (group: IbsCbsGroupedItem) => {
...
export default AbaClassificacao;
