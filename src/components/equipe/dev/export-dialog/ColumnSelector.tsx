...
export function ColumnSelector(props: Props) {
  return <>
    <div className="flex items-center gap-2 mb-3 flex-wrap">
      <Select value={props.selectedProfileId || "__none__"} onValueChange={(value) => props.onLoadProfile(value === "__none__" ? "" : value)}><SelectTrigger className="w-[200px] min-w-0"><SelectValue placeholder={props.loadingProfiles ? "Carregando..." : "Carregar Preset"} /></SelectTrigger><SelectContent><SelectItem value="__none__">Nenhum</SelectItem>{props.profiles.map((profile) => <SelectItem key={profile.id} value={profile.id}><span className="flex items-center gap-2">{profile.is_default && <Star className={`h-3 w-3 ${TOM_DA_ESTRELA}`} />}{profile.name}</span></SelectItem>)}</SelectContent></Select>
      <ToolButton label="Salvar" onClick={props.onSave}><Save className="h-4 w-4" /></ToolButton>
      {props.selectedProfileId && <><ToolButton label="Favoritar" onClick={props.onDefault} disabled={props.defaultPending}><Star className={`h-4 w-4 ${props.isSelectedProfileDefault ? TOM_DA_ESTRELA : ""}`} /></ToolButton><ToolButton label="Excluir" onClick={() => props.onDelete(props.selectedProfileId)} destructive><Trash2 className="h-4 w-4" /></ToolButton></>}
    </div>
