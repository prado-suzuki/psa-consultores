...
export function EFDExportStatus({ status, message, count, onCancel, onExport }: Props) {
  ...
  return <DialogFooter className="p-5 border-t border-slate-200 flex justify-between items-center bg-white flex-shrink-0">
    <div className="flex items-center gap-3">
      {processing ? <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center"><Server className="h-5 w-5 text-amber-600 animate-pulse"/></div> : status ==='completed'? <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center"><CheckCircle2 className="h-5 w-5 text-emerald-600"/></div> : <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-primary font-bold">{count}</div>}
