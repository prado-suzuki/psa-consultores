...
export function EFDExportProfiles(props: Props) {
  ...
  return <div className="flex flex-wrap gap-4 items-end justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm mt-4">
    ...
    <div className="flex items-center gap-4 border-l border-slate-200 pl-4 h-10"><button onClick={props.onSelectAll} className="text-sm font-bold text-primary hover:underline">Selecionar Todos</button><button onClick={props.onClear} className="text-sm font-bold text-slate-500 hover:text-red-500 hover:underline">Limpar</button></div>
  </div>;
}
