...
import { TOM_DA_ESTRELA } from '@/components/equipe/dev/classesDoPerfil';

/** A estrela da lista: o tom do mapa mais o tamanho daquele lugar. */
const ESTRELA_PADRAO = `h-3 w-3 ${TOM_DA_ESTRELA}`;
    ...
    <div className="flex-1 min-w-[280px]">
      ...
      <div className="flex gap-2">
        <Select value={props.selectedProfile} onValueChange={props.onApply}>
          ...
          <SelectContent>
            <SelectItem value="none" className="text-muted-foreground">Nenhum</SelectItem><SelectItem value="all">Todos os Registros</SelectItem>
            {props.profiles.length > 0 && <div className="h-px bg-muted my-1"/>}
            {props.profiles.map(profile => <div key={`user_${profile.id}`} className="relative flex items-center pr-10 group">
              <SelectItem value={`user_${profile.id}`} className="flex-1"><span className="flex items-center gap-2">{profile.is_default && <Star className={ESTRELA_PADRAO} />}{profile.name}</span></SelectItem>
        ...
        {props.profiles.length > 0 && <Button variant="outline" size="icon" className="h-11 w-11" title={!selectedId ? 'Selecione um perfil para favoritar' : props.selectedIsDefault ? 'Perfil padrão' : 'Definir como padrão'} onClick={() => selectedId && props.onDefault(selectedId)} disabled={props.defaultPending || !selectedId}><Star className={cn('h-5 w-5', props.selectedIsDefault && TOM_DA_ESTRELA)} /></Button>}
      </div>
    </div>
    <div className="flex items-center gap-4 border-l border-border pl-4 h-10"><button onClick={props.onSelectAll} className="text-sm font-bold text-primary hover:underline">Selecionar Todos</button><button onClick={props.onClear} className="text-sm font-bold text-muted-foreground hover:text-destructive hover:underline">Limpar</button></div>
  </div>;
}
