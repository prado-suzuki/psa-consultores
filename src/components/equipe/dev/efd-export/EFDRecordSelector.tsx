...
export function EFDRecordSelector({ blocosDisponiveis, expanded, selected, onToggleBlockOpen, onToggleBlock, onToggleRecord }: Props) {
  return <div className={cn('flex-1 overflow-y-auto p-6 bg-slate-50','[&::-webkit-scrollbar]:w-3','[&::-webkit-scrollbar-thumb]:bg-slate-400','[&::-webkit-scrollbar-thumb]:rounded-full')}>
    <div className="space-y-3">{Object.entries(blocosDisponiveis).map(([block, records]) => {
      ...
      return <div key={block} className={cn('border border-slate-200 rounded-xl overflow-hidden bg-white transition-all duration-300', isExpanded &&'shadow-sm')}>
        <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-slate-50 transition-colors"onClick={() => onToggleBlockOpen(block)}>
          <div className="flex items-center gap-3">
            <div className="p-1 rounded hover:bg-slate-200"onClick={event => { event.stopPropagation(); onToggleBlock(block); }}>
        ...
        <div className={cn('overflow-hidden transition-all duration-300', isExpanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0')}>
          <div className="p-4 bg-slate-50 border-t border-slate-100">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">{records.map(record => {
              ...
              return <label key={record.codigo} className={cn('flex items-start gap-2 cursor-pointer p-2 rounded-lg transition-colors border', isSelected ?'bg-emerald-50 border-emerald-200':'bg-white border-transparent hover:border-slate-200')}>
