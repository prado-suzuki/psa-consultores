...
const PROCESSO: Bloco[] = [
    ...
    linhas: [
      {
        ...
        fontes: ['frequencia — Processos (editar metadados)'],
  ...
  },
  {
    ...
    intro: 'Somatório das etapas do processo. Cada etapa é ponderada pelo seu volumePorProcesso.',
    linhas: [
      {
        indicador: 'Horas / execução',
        formula: 'Σ etapas [ (Σ executadoPor.horas + Σ revisadoPor.horas) × volumePorProcesso ]',
        fontes: ['executadoPor[].horas, revisadoPor[].horas, volumePorProcesso — Etapas (Mapear → Como era)'],
      },
      {
        indicador: 'Custo de pessoas / execução',
        formula: 'Σ etapas [ (Σ horas × responsavel.custoHora) × volumePorProcesso ]',
        fontes: ['horas — Etapas', 'custoHora — Responsáveis'],
        nota: 'Responsável com custoHora = 0 (externo/cliente) entra com custo zero; vínculo perdido cai no custo/hora médio.',
      },
      {
        indicador: 'Custo de sistemas / ano',
        formula: 'Σ sistemas usados (custo fixo/licença mensal × 12 × rateio%)',
        fontes: ['sistemas[] — Etapas (rateio % por sistema)', 'custoVariavelPorUso (custo fixo/licença) — Sistemas'],
      ...
      },
      {
        ...
        formula: 'Σ etapas [ custoPessoasDaEtapa × taxaRetrabalho ] × execuções/ano',
        fontes: ['taxaRetrabalho — Etapas', 'custo de pessoas da etapa (acima)'],
      },
      {
    ...
    linhas: [
      {
        ...
        formula: 'Σ melhoria.horasTreinamento × custoHora médio',
        fontes: ['horasTreinamento — Melhorias', 'custoHora — Responsáveis'],
      },
      {
        indicador: 'Execução de melhorias',
        formula: 'Σ melhoria.executadoPor[].horas × custoHora',
        fontes: ['executadoPor[].horas — Melhorias', 'custoHora — Responsáveis'],
  ...
  },
  {
    ...
    linhas: [
      ...
      {
        indicador: 'ROI (%)',
        formula: 'economiaAnual / investimento × 100',
        fontes: ['derivado'],
        nota: 'Investimento zero → ROI indefinido (—). Pode ser negativo. No Dashboard, o ROI exibido usa o horizonte selecionado: (economiaMensal × horizonte) / investimento.',
      },
      {
        indicador: 'Payback (meses)',
        formula: 'investimento / (economiaAnual / 12)',
        fontes: ['derivado'],
...
const DASHBOARD: Bloco[] = [
  ...
  },
  {
    ...
    linhas: [
      { indicador: 'Custo operacional / ano', formula: 'Σ processos custoAnual', fontes: ['derivado (aba ROI do processo)'] },
      { indicador: 'Horas alocadas / ano', formula: 'Σ processos horasAnual', fontes: ['derivado'] },
      { indicador: 'Custo de retrabalho / ano', formula: 'Σ custosCategoria.retrabalho', fontes: ['taxaRetrabalho — Etapas'] },
      { indicador: 'Retrabalho médio %', formula: 'média de taxaRetrabalhoMedia entre processos', fontes: ['taxaRetrabalho — Etapas'] },
      {
      ...
      },
      {
        ...
        formula: 'horas = Σ gargalo.horasGastas · custo = horasGastas × custoHora médio · agrupado por origem',
        fontes: ['horasGastas, origem, processos[] — Gargalos', 'custoHora — Responsáveis'],
      },
    ],
  },
  {
    titulo: '3. Melhorias e investimento',
    linhas: [
      {
        indicador: 'Composição do investimento',
  ...
  },
  {
    ...
    linhas: [
      { indicador: 'Economia anual', formula: 'Σ economiaAnual dos processos (pode ser negativa)', fontes: ['derivado'] },
      { indicador: 'ROI / Resultado líquido (horizonte)', formula: 'ROI = (economiaMensal × horizonte) ÷ investimento · Resultado líquido = economiaMensal × horizonte − investimento', fontes: ['Horizonte — filtro do Dashboard'], nota: 'KPIs recalculam ao trocar o horizonte (12/24/36 meses); ROI e resultado líquido podem ser negativos antes do payback.' },
      { indicador: 'Payback', formula: 'investimentoTotal ÷ (economia/12)', fontes: ['derivado'] },
      { indicador: 'Curva break-even', formula: 'economia mensal acumulada × horizonte (12/24/36) vs. investimento', fontes: ['Horizonte — filtro do Dashboard'] },
    ],
  },
];
...
const SETOR: Bloco[] = [
  {
    ...
    linhas: [
      {
        indicador: 'Snapshot',
        formula: 'fotografia de custoAnual, horasAnual, economiaAnual, horasLiberadas, roiPercentual e payback no momento do salvamento',
  ...
  },
  {
    ...
    linhas: [
      { indicador: 'Δ custo', formula: 'baseline.custoAnual − última.custoAnual', fontes: ['custoAnual (snapshot)'] },
      { indicador: 'Δ %', formula: 'Δ custo / baseline.custoAnual × 100', fontes: ['custoAnual (snapshot)'] },
      { indicador: 'Δ horas', formula: 'baseline.horasAnual − última.horasAnual', fontes: ['horasAnual (snapshot)'] },
    ],
  },
  {
    ...
    linhas: [
      ...
      { indicador: 'ROI médio do portfólio', formula: 'média de roiPercentual das últimas medições', fontes: ['roiPercentual (snapshot)'] },
      {
        indicador: 'Série temporal',
        formula: 'em ordem cronológica, acumula economiaAnual/12 de cada snapshot mês a mês',
        fontes: ['economiaAnual, snapshotEm (snapshot)'],
      },
    ],
  },
];
...
const ABAS: { id: Aba; label: string; intro: string; blocos: Bloco[] }[] = [
  { id: 'processo', label: 'ROI do processo', intro: 'Cálculo de um único processo (wizard "Configurar ROI"). Compara Como Era × Como Ficou e atribui o investimento das melhorias. No diagnóstico, cada item pendente é clicável e leva direto ao campo de origem — a etapa exata no editor, ou o modal de detalhe do sistema, responsável, melhoria ou gargalo.', blocos: PROCESSO },
