...
const ConsultaEFDICMS = () => {
  ...
  return <DevLayout tela="efdIcms"><TooltipProvider delayDuration={300}>
    <DevPageHeader description="A **EFD ICMS/IPI** reúne as Escriturações Fiscais Digitais do ICMS e do IPI entregues ao SPED. Usa os arquivos da base, filtrados por cliente, contribuinte e período. Consulte blocos e registros em tela, baixe os arquivos originais em lote (.zip) ou exporte para Excel (.xlsx)." />
    <ConsultaEfdFilters clientes={domain.clientes} contribuintes={domain.contribuintes} loadingClientes={domain.loadingClientes} loadingContribuintes={domain.loadingContribuintes} cliente={cliente} contribuinte={contribuinte} inicio={inicio} fim={fim} searching={overviewQuery.isLoading && searchTriggered} onCliente={changeCliente} onContribuinte={changeContribuinte} onInicio={setInicio} onFim={setFim} onClear={clear} filtrosAtivos={[cliente, contribuinte, inicio?.month !== defaults.inicio.month || inicio?.year !== defaults.inicio.year || fim?.month !== defaults.fim.month || fim?.year !== defaults.fim.year].filter(Boolean).length} onSearch={search} />
