...
import DashboardUsoEnvio from './DashboardUsoEnvio';
...
vi.mock('@/components/equipe/dev/DevLayout', () => ({
  DevLayout: ({ children }: { children: React.ReactNode }) => <main>{children}</main>,
}));
...
describe('DashboardUsoEnvio', () => {
  ...
  it('mantém a consulta de arquivos desabilitada até abrir a aba', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/equipe/dev/gerenciar-dados/uso-envio']}>
        <DashboardUsoEnvio />
      </MemoryRouter>,
    );
  ...
  it('uma pessoa na URL prevalece sobre uma ferramenta antiga', () => {
    render(
      <MemoryRouter
        initialEntries={['/equipe/dev/gerenciar-dados/uso-envio?usuario=Pessoa+A&ferramenta=Mapa']}
      >
        <DashboardUsoEnvio />
      </MemoryRouter>,
    );
