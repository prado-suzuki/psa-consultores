...
const SIT_ESP_MAP: Record<string, string> = {
  '0': 'Normal',
  '1': 'Extinção',
  '2': 'Fusão',
...
const ConsultaECF = () => {
  ...
  const handleDownloadTxt = async (arquivo: EFDArquivo) => {
    ...
    try {
      const url = getApiUrl(`/api/v1/query/download/efd/ecf/arquivo/${encodeURIComponent(arquivo.ID_ARQUIVO)}`);
      ...
      const fileName = `ECF_${arquivo.CNPJ}_${dtIni}.txt`;
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      let filename = isZip ? `ECF_${cnpjContribuinte}.zip` : `ECF_${cnpjContribuinte}.txt`;
                    ...
                    <tr key={arquivo.ID_ARQUIVO} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <Badge variant="outline" className="text-[10px] font-bold">
                          {arquivo.IND_SIT_ESP ? (SIT_ESP_MAP[arquivo.IND_SIT_ESP] || arquivo.IND_SIT_ESP) : 'Normal'}
                        </Badge>
                      </td>
