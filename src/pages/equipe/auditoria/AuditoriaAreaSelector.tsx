...
const CAIXA_DO_SELO = 'bg-gradient-to-br from-[#406d77] to-[#355a62] ring-1 ring-[#406d77]/40';

const AuditoriaAreaSelector = () => {
  const navigate = useNavigate();
  const { isAdmin } = useAuth();
  const { categories, isLoading: carregandoCategorias } = useUserAccessibleCategories();
  const { hasAccess: podeVerOWork, isLoading: carregandoWork } = usePageAccess('/equipe/auditoria/work');
  ...
  const todos: CartaoDeAmbiente[] = [
    {
      id: 'projects',
      label: AREAS.auditoriaProjects.nome,
      description: 'Projetos, tarefas e clientes da área de Auditoria',
      path: '/equipe/auditoria/inicio',
      iconNode: <ClipboardCheck className="h-6 w-6 text-primary-foreground" />,
      visivel: isAdmin || !!categories?.includes('auditoria'),
    },
    {
      id: 'work',
      label: AREAS.auditoriaWork.nome,
      description: 'Ferramentas e aplicações desenvolvidas para a Auditoria',
      path: '/equipe/auditoria/work',
    ...
    <div className="min-h-screen dark bg-background text-foreground flex items-center justify-center p-4 overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold mb-2">{AREAS.auditoria.subtitulo}</h1>
...
export default AuditoriaAreaSelector;
