...
| `cenario-apresentacao-familia-prado.sql` | Cliente `[APR] Família Prado Agronegócios`: holding familiar com o cadastro completo (quatro pessoas, proprietária com três imóveis aprovados e um pendente, controladora vazia) e **nada registrado na junta**, para apresentar a constituição, as alterações do quadro e a concentração das quotas. Põe também o prefixo `[APR]` nos clientes da apresentação de 23/09/2026. **Reaplicar é o botão de reset**: apaga tudo do cliente, inclusive peças registradas e histórico de auditoria, e recria o cadastro. Aborta sem apagar nada se o cliente tiver dados numa tabela que o reset não cobre. |
