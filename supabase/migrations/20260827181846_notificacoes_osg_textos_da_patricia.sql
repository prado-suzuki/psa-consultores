-- 20260827181846_notificacoes_osg_textos_da_patricia.sql
-- Os textos dos tres avisos internos da OSG, revisados pela Patricia em
-- 27/08/2026. So texto: nenhuma regra, nenhuma coluna, nenhuma permissao muda.
--
-- Evento 1  titulo mantido, corpo reescrito em voz passiva.
-- Evento 2  titulo passa a nomear o ATO ("Cobranca de documentos pendentes") em
--           vez do objeto ("Documentos cobrados do cliente"). O corpo encurta
--           para "Cobranca enviada ao cliente." porque o resto vem da TELA, em
--           `_detalhe`, com os numeros que o analista esta vendo na hora.
-- Evento 3  "encerrada" vira "finalizada", palavra escolhida por ela.
--
-- O rotulo da thread e a etiqueta do sino acompanham, no front. A etiqueta do
-- evento 3 dizia "Documento aprovado", que era o nome TECNICO do tipo
-- (`notificacao_tipo = documento_aprovado`) vazando para a tela: o gatilho do
-- evento sempre foi o encerramento, nao uma aprovacao de documento. O valor do
-- enum NAO e renomeado, porque ele e o `event_type` que a borda e o n8n usam.
--
-- Reversao: reaplicar 20260826202804.
...
BEGIN
  ...
  CASE _evento
    WHEN 'solicitacao_enviada' THEN
      ...
      v_corpo  := 'A lista de documentos foi enviada ao cliente e o acesso ao portal foi liberado.';
    WHEN 'situacao_documentos' THEN
      v_tipo   := 'cobranca_pendencia';
      v_kind   := 'documentos_cobrados';
      v_titulo := 'Cobrança de documentos pendentes';
      -- O corpo termina aqui e o resto vem da TELA, em `_detalhe`: os numeros tem
      -- que ser os que o analista esta vendo no momento do envio.
      v_corpo  := 'Cobrança enviada ao cliente.';
    WHEN 'documento_aprovado' THEN
      ...
      v_corpo  := 'Documentação conferida e validada. Solicitação finalizada.';
    ELSE
