-- A leitura da fase de CHECKLIST no portal do cliente: uma linha por
-- (documento pedido × entidade), com o que gravar no arquivo e o que já chegou.
--
-- POR QUE UMA RPC NOVA, E NÃO EVOLUIR get_solicitacao_ativa_cliente
--
-- São dois momentos diferentes do processo, e a de lá continua servindo o seu:
--
--   solicitação inicial  → get_solicitacao_ativa_cliente → 4 gavetas, balde,
--                          nome de documento sem repetir
--   depois da virada     → esta função                   → checklist: o que
--                          falta, de quem, com upload na própria linha
--
-- Na solicitação inicial o cadastro do cliente costuma estar VAZIO: é da primeira
-- leva de arquivos que a PSA monta pessoas e imóveis. Não existe instância para
-- multiplicar antes disso, e por isso a gaveta-balde é a ferramenta certa da
-- primeira fase. Mexer na leitura de lá impactaria o fluxo em execução sem ganho.
--
...
--   não se aplica = solicitacao_item_nao_aplicavel (item, instância): não vira linha
--   falta         = o resto
--
-- POR QUE A MULTIPLICAÇÃO MORA AQUI DENTRO
--
-- O portal não tem (e não ganha) policy de leitura em `pessoa`, `bem` ou
-- `matricula`. Sendo SECURITY DEFINER, esta função escolhe exatamente o que sai do
-- cadastro: o id e o NOME da entidade, mais nada. Multiplicar no front exigiria
-- abrir essas três tabelas ao cliente, que é a versão caríssima da mesma coisa.
--
-- DUAS DECISÕES QUE ESTÃO NO CORPO, PARA NÃO SEREM LIDAS COMO DETALHE
--
--   1. Arquivo que a PRÓPRIA PSA subiu (`fonte <> 'cliente'`) CONTA como recebido.
--      Se não contasse, o cliente veria pendência de documento que já está com a
--      gente e reenviaria. Ele vem sinalizado em `recebido_interno`, e o arquivo em
--      si NÃO é exposto: a lista `arquivos` só traz os de `fonte = 'cliente'`, que
--      são os que ele já vê pela policy do portal. Reverter é apagar um predicado.
--   2. Item DISPENSADO e instância marcada como "não se aplica" não viram linha.
--      Mostrar riscado transformaria a lista do cliente em relatório do trabalho
--      interno da PSA.
--
-- O QUE ESTA FUNÇÃO NÃO FAZ: quantidade. "IRPF dos 3 últimos exercícios" fecha no
-- primeiro arquivo, porque não existe quantidade esperada no modelo (decisão do
-- plano da solicitação). Quem diz "três" é a `nota`.
--
-- `ambiente` de propósito NÃO é filtrado em `documento_arquivo`, para acompanhar
-- os leitores que já existem (useDocumentosByCliente e a lista do portal). Filtrar
-- só aqui faria o checklist discordar da lista de arquivos que o cliente vê.
--
-- Reversão: DROP FUNCTION public.get_pendencias_documentos_cliente();
...
AS $function$
  ...
  -- A solicitação da fase de checklist. `em_checklist` na frente porque é a viva;
  -- a última `encerrada` fica como retrato, para a tela não zerar no fechamento.
  sol AS (
  ...
  -- As instâncias do cliente, no vocabulário do grão. `nome` é o único campo do
  -- cadastro que sai daqui.
  inst AS (
    ...
    UNION ALL
    -- Matrícula "do cliente" é a do bem dele OU a que ele titulariza, mesmo
    -- critério de ClassificarDocumentos e OrganizarDocumentos. Rural sai de
    -- `matricula.tipo_bem = 'IR'`, como no checklist do consultor.
    SELECT 'matricula',
           m.id,
    ...
    UNION ALL
    -- O grão agregado: arquivo sem dono. `id` nulo de propósito, é o formato do
    -- alvo no front (kind 'cliente' não tem id).
    SELECT 'cliente', NULL::uuid, 'Documentos gerais', NULL, 'cliente' FROM cli
  ),
  -- Os itens ativos do pedido, com texto herdado e o tipo resolvido: o do
  -- catálogo, ou o avulso que nasceu do item pedido à mão (migration 20260807150000).
  itens AS (
    SELECT i.id,
  ...
  -- Os arquivos classificados do cliente, com o dono normalizado no mesmo
  -- vocabulário das instâncias.
  arq AS (
    SELECT da.id,
           da.nome_original,
           da.created_at,
