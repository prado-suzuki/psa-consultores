...
-- 1a. Dedup snapshots ROI MAPA: mantém 1 por process_id (mais recente)
...
-- 1b. Snapshot legado snapshot_at NULL → data antiga
UPDATE public.process_scenarios SET snapshot_at='2025-01-01T00:00:00Z'
WHERE name='reduzir para 2hs' AND snapshot_at IS NULL;

-- 1c. Snapshots ROI inferidos para 17 processos PSA
DO $ins$
DECLARE
  v_cb uuid := COALESCE((SELECT created_by FROM public.process_scenarios WHERE created_by IS NOT NULL LIMIT 1), 'fb81a718-124e-45e2-bab5-b0241738c7b7'::uuid);
  v_pid uuid;
  v_code text;
  arr_ids uuid[] := ARRAY[
    'bfd4a06a-de80-4fbe-9955-a877a551a3dc'::uuid,'48d2e792-3fd2-41c1-a357-582a553d38d9'::uuid,
    'c7db1b56-22cc-4c8d-b36a-b280f8944172'::uuid,'8895f320-b43d-4be4-8cd3-a844b8ec5531'::uuid,
    '6a4c1a83-bfde-464e-a355-0308c8317bb1'::uuid,'2e4a32eb-299b-4a16-8715-c25aadab0e38'::uuid,
    'fc0233c3-08b5-4428-be6f-332634cc9c24'::uuid,'fd3d188d-1582-4d1d-96a2-7a73fd04de3d'::uuid,
    '6e19cf21-4bf4-401c-85bd-1cd336c91702'::uuid,'5c2c1c02-e51e-4790-b251-15f2306ca545'::uuid,
    'cd6b15c3-9898-4a93-82a7-2c17a831703c'::uuid,'ec58feca-8367-409f-8cd6-b880644896b6'::uuid,
    'a257db7d-12ff-4344-8bb6-ccc2c7dfb610'::uuid,'72564838-3c39-4fc2-90f8-f046c33a259b'::uuid,
    '2b57fa7e-a953-4afc-b1b8-176333abe08e'::uuid,'5a81e296-b93e-4162-97f3-260f6524fa6d'::uuid,
    'd814a6d7-5b6d-41c0-bd64-8af78f187775'::uuid
  ];
BEGIN
  FOREACH v_pid IN ARRAY arr_ids LOOP
    SELECT code INTO v_code FROM public.processes WHERE id = v_pid;
    ...
    SELECT gen_random_uuid(), v_pid, v_cb, 'Snapshot ROI MAPA — '||v_code, '{}'::jsonb, 'efficiency', 'time', '2026-06-08T12:00:00Z', 10560, 192, 3168, 400, 2.4, 58, 634, NOW()
    WHERE NOT EXISTS (SELECT 1 FROM public.process_scenarios WHERE process_id=v_pid AND name='Snapshot ROI MAPA — '||v_code);
  END LOOP;
END $ins$;

-- 2. Projetos PSA: status + description + justificativas
...
UPDATE public.projects SET status='Mapeamento', description='Iniciativa transversal de organização da rotina interna do PSA Consultores e integração das ferramentas digitais com a gestão de projetos.', updated_at=NOW() WHERE id='3489ea0a-6d60-41c3-9784-591aa0c65572';
...
UPDATE public.projects SET status='ROI', description='Automação completa da quebra de SPEDs (EFD Contribuições, EFD ICMS/IPI, ECD, ECF, CT-e, NF-e) — substitui o processo manual no TaxSheets pela ferramenta Consulta SPED.', updated_at=NOW() WHERE id='4cb03075-d8a7-4062-9d51-a9c64dc47523';
...
UPDATE public.projects SET status='ROI', description='Plataforma de levantamento e apuração de créditos de PIS/COFINS — consolida débitos, créditos, exclusões e cruzamento de bases.', updated_at=NOW() WHERE id='cc5ef9ae-0425-4b4b-8035-c10af1d6bef6';
...
UPDATE public.projects SET status='ROI', description='Dashboard analítico e operacional para controle dos saldos de PIS/COFINS/IPI e pedidos PERDCOMP — substitui planilha de controle por cliente.', updated_at=NOW() WHERE id='492df2ea-51f3-47cf-b602-3af69e1bd1b3';
...
UPDATE public.projects SET status='ROI', description='Automação das consultas tributárias em lote (NCM, CNPJ Simples Nacional, CNAE) — substitui consultas uma a uma no Econet/Portal SN.', updated_at=NOW() WHERE id='280035db-24c6-4c1c-9374-474acfb5663d';
...
UPDATE public.projects SET status='Melhorias', description='Dashboards de Power BI para controle de contratos fixos, prazos e distribuição de carga da equipe — substitui controle informal e planilhas pessoais.', updated_at=NOW() WHERE id='1de4789f-50ce-40bd-945a-2ae01eba5b21';
...
UPDATE public.projects SET status='ROI', description='Ferramenta de classificação de produtos por NCM com sincronização das decisões DIFAL — automação completa da apuração do diferencial de alíquota.', updated_at=NOW() WHERE id='267dd955-3be2-4b51-ab14-c8237ecbaa3a';
...
UPDATE public.projects SET status='Mapeamento', description='Iniciativa de adequação à reforma tributária: simulador e ferramentas de cálculo de IBS/CBS para apoio aos clientes na transição.', updated_at=NOW() WHERE id='20c83382-dbdf-49ef-a915-ca97dcd53a64';
...
UPDATE public.projects SET status='Melhorias', description='Padronização dos papéis de trabalho contábeis e fiscais (DIRPF, PIS/COFINS, IRPJ/CSLL, LCDPR, DIFAL) — biblioteca de templates da Prado.', updated_at=NOW() WHERE id='d1a9fabd-8262-429e-8501-7a08a6d43ad8';
...
UPDATE public.projects SET status='Melhorias', description='Portal externo do PSA Consultores + canal de chamados/onboarding de novos clientes, substituindo o controle manual por e-mail.', updated_at=NOW() WHERE id='6e237fa4-4507-4137-84f6-0d11db63d46f';
...
-- 3. Processos: frequency normalizado + description
UPDATE public.processes SET frequency='Mensal', description='Recepção de novos clientes e solicitação dos documentos necessários para iniciar o trabalho.', updated_at=NOW() WHERE id='bfd4a06a-de80-4fbe-9955-a877a551a3dc';
UPDATE public.processes SET frequency='Mensal', description='Controle dos contratos fixos com clientes e acompanhamento dos prazos legais de obrigações tributárias.', updated_at=NOW() WHERE id='48d2e792-3fd2-41c1-a357-582a553d38d9';
UPDATE public.processes SET frequency='Diária', updated_at=NOW() WHERE id='c7db1b56-22cc-4c8d-b36a-b280f8944172';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='8895f320-b43d-4be4-8cd3-a844b8ec5531';
UPDATE public.processes SET frequency='Mensal', description='Revisão específica de PIS/COFINS no contexto dos templates de papéis de trabalho.', updated_at=NOW() WHERE id='6a4c1a83-bfde-464e-a355-0308c8317bb1';
UPDATE public.processes SET frequency='Mensal', description='Revisão específica do bloco LCDPR no contexto dos templates de papéis de trabalho.', updated_at=NOW() WHERE id='2e4a32eb-299b-4a16-8715-c25aadab0e38';
UPDATE public.processes SET frequency='Mensal', description='Recebimento, atribuição, acompanhamento e entrega dos projetos tributários pontuais (recuperações, defesas RFB, revisões).', updated_at=NOW() WHERE id='3d896cea-f554-42f4-a7b9-8ec61195c71d';
UPDATE public.processes SET frequency='Quinzenal', updated_at=NOW() WHERE id='fc0233c3-08b5-4428-be6f-332634cc9c24';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='fd3d188d-1582-4d1d-96a2-7a73fd04de3d';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='6e19cf21-4bf4-401c-85bd-1cd336c91702';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='5c2c1c02-e51e-4790-b251-15f2306ca545';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='cd6b15c3-9898-4a93-82a7-2c17a831703c';
UPDATE public.processes SET frequency='Diária', updated_at=NOW() WHERE id='ad8a6b69-2579-4a16-b708-6319555a87f9';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='ec58feca-8367-409f-8cd6-b880644896b6';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='a257db7d-12ff-4344-8bb6-ccc2c7dfb610';
UPDATE public.processes SET frequency='Mensal', description='Revisão específica de IRPJ/CSLL no contexto dos templates de papéis de trabalho.', updated_at=NOW() WHERE id='72564838-3c39-4fc2-90f8-f046c33a259b';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='2b57fa7e-a953-4afc-b1b8-176333abe08e';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='5a81e296-b93e-4162-97f3-260f6524fa6d';
UPDATE public.processes SET frequency='Mensal', updated_at=NOW() WHERE id='d814a6d7-5b6d-41c0-bd64-8af78f187775';

-- 4. Etapas: execution + remover etapa lixo
DELETE FROM public.process_stages WHERE id='9ee164f2-42b5-46aa-90fd-29d7c9a6eeb8';
UPDATE public.process_stages SET execution='manual', updated_at=NOW() WHERE id IN (
  'fc9db04d-e726-44e6-9137-f2a5b35bc6d5','9dd655a5-f649-4f56-a605-18d85e10c36a','680f4619-70c1-481a-ba0d-934b1af2e45e',
  '109ef44a-027d-4258-b81a-2fed68addda6','63722899-2085-4199-8a0c-87f83d2b970f','570a5a69-cff0-4d46-972d-a06959ab7218',
  '84b54c96-d8b5-4b12-8813-549995291a15','c130a6b8-95d8-459d-8886-afada016927f','7a0e58e4-0197-412f-832d-064cda83708b',
  'e8f3348b-dd11-4048-937c-46c3a98684a1','8973f3f9-5c85-4470-add5-31981aa19437','4b411282-c34e-44f4-b813-d26a9a4dbb39',
  'b16de884-0dde-46f8-b893-6441ef01281c','d4d172d6-2859-4ecc-8393-9ec77e1f555a','4e8f8f40-29c9-4687-baf1-a70de32d088f',
  'd12521e8-f0ed-40db-a4de-f346513defe6','36af4652-e245-4cf9-9738-c8e67bbe408f','93769eab-ab2c-4b04-8d54-11a03f383e95',
  '0f520637-68a5-4c84-b609-86097a5dd9d3','13e7def5-794a-4217-be89-b6029c6f3f37','f38f8d5f-cee7-46c3-9baa-84d9c731ead3',
  '2e14d4ac-f4f6-4409-991f-956eaf135c40','764fefec-f95f-4786-85a9-6702fb495e47','1f019621-2980-4240-8b0a-42061c4938af',
  '7cb8d96b-05ea-4ff5-a785-d14d55db65f2','ba8e9684-296f-4bcf-8cee-14c4f97cbbde','6798e792-9a37-4e90-b1c9-bbbe64f58dab',
  '3b27810c-53b4-4859-bd23-31283906768a','f643839c-949f-4917-b179-05a2760f6d28','8084e792-3880-440c-9e90-2554b570f051',
  '3aa0bd87-0f5d-483a-8f06-458703c89acb','6e8630c2-d62a-4cf0-916e-e735283103b4','bab99a89-2f6b-4ced-a5b1-146c8fa10bfd',
  'b9465da6-6183-4c9e-900d-7a926a974030','90574f89-15fb-4638-8541-4ee375ff1038'
);
UPDATE public.process_stages SET execution='semi_automatica', updated_at=NOW() WHERE id IN (
  '9a5190b0-890f-4c3c-a6b9-2d1331a032e3','3b94d3d1-9e37-4512-9e22-aa37c965d95c','3068c6a7-de8a-4515-ade3-71319a33ff79',
  'b3a2c71f-8446-498a-97ec-027da407a5fc','8ae27d4f-5bd9-4495-9a08-36b7dfccc73c','c4a3c310-e3e0-4804-8612-835afc5badb1',
  '752f4285-43d2-449c-ab4a-a6821e9a17dc','90d74295-8465-4cb2-86d0-316afb116c5d','36a595d3-900b-40ad-b13f-a563e965f45b',
  'dfe736cf-721f-49de-a757-54a031b86d43','cb5d1b20-0cbb-4015-b72a-acdec2773ab4','84c11fb7-61cf-41c5-bff8-a21f6bce844b',
  'ef8b06f6-7785-45a9-938e-0570ce6b15a3','51c6303f-b96f-466c-9599-9e8340321bfc','5f9ae7be-d7a9-4a33-adb9-42f7992f8eeb'
);
INSERT INTO public.etapa_responsaveis (id, etapa_id, scenario, responsavel_id, papel, horas, created_at)
SELECT gen_random_uuid(), v.etapa_id, 'AS-IS', 'c607fbc2-df49-4dcf-8b5c-dda3064c2c55'::uuid, 'executado', 2, NOW()
FROM (VALUES
  ('3b94d3d1-9e37-4512-9e22-aa37c965d95c'::uuid),('b3a2c71f-8446-498a-97ec-027da407a5fc'::uuid),
  ('752f4285-43d2-449c-ab4a-a6821e9a17dc'::uuid),('90d74295-8465-4cb2-86d0-316afb116c5d'::uuid),
  ('84c11fb7-61cf-41c5-bff8-a21f6bce844b'::uuid),('ef8b06f6-7785-45a9-938e-0570ce6b15a3'::uuid)
) v(etapa_id)
ON CONFLICT (etapa_id, scenario, responsavel_id, papel) DO NOTHING;

-- 5. Documentos normalização (PSA cluster)
...
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Working Paper da Prado — planilha-modelo usada como suporte das análises tributárias e contábeis.', updated_at=NOW() WHERE id='00e38899-f1a6-cfbe-b770-6667e9382a51';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Centro Virtual de Atendimento da Receita Federal — acesso a PER/DCOMP, situação fiscal e obrigações acessórias.', updated_at=NOW() WHERE id='2f4075f4-5df3-9057-d1d0-7ea7fd6e012f';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Sistema da RFB para envio de Pedido Eletrônico de Restituição/Ressarcimento e Declaração de Compensação.', updated_at=NOW() WHERE id='5f68b103-90f8-5558-3326-497ebe6ace09';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=30, custo_licenca_mensal=0, tipo_custo=NULL, descricao='E-mail corporativo para comunicação com clientes e equipe.', updated_at=NOW() WHERE id='3d7ed977-3d89-20e3-da1e-5d2ef26ce8d6';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Ferramenta interna do PSA Consultores para apoio às análises.', updated_at=NOW() WHERE id='9ccad270-2df1-7d34-5c29-8273981cfaa1';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=300, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Plataforma de consulta de legislação tributária (assinatura paga).', updated_at=NOW() WHERE id='89d562ad-01e2-4eda-3f8d-cf43442dd2e6';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=40, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Editores de documentos (Microsoft Word) para minutas e pareceres.', updated_at=NOW() WHERE id='069ebde5-f8b4-13ff-79fc-0220b45a08c3';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Portais estaduais de consulta de NF-e e situação fiscal estadual.', updated_at=NOW() WHERE id='299bddfc-f10c-dbd0-a7ad-cb5d7e6b8b33';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Portal da Receita para consulta de enquadramento no Simples Nacional.', updated_at=NOW() WHERE id='7c25714e-226c-985b-82f0-0df85ea3c78f';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Programa oficial para Livro Caixa Digital do Produtor Rural.', updated_at=NOW() WHERE id='bb0f336e-dd90-355b-769a-d1568b261059';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Programa oficial da Receita Federal para Declaração de Imposto de Renda Pessoa Física.', updated_at=NOW() WHERE id='f282c0cf-a953-dd2c-2f4e-439d5b39a23a';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Canais de comunicação informal entre equipe (urgências, atribuições).', updated_at=NOW() WHERE id='3043408d-66d9-50cb-5448-1e138cf8161e';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=0, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Comunicação verbal/presencial com cliente ou equipe.', updated_at=NOW() WHERE id='7d79ed8f-db53-5186-6a95-c8cb17ac589d';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=150, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Ferramenta de conversão de PDF para Excel (Able2Extract ou similar).', updated_at=NOW() WHERE id='296c32d6-63d9-56fb-b7bf-b4495bb110b9';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=40, custo_licenca_mensal=0, tipo_custo=NULL, descricao='Excel utilizado para simulações tributárias e projeções.', updated_at=NOW() WHERE id='26edf2b2-d600-f4f7-f5a0-94eed4c82aaf';
UPDATE public.sistemas_processo SET custo_variavel_por_uso=40, custo_licenca_mensal=0, tipo_custo=NULL, descricao='PowerPoint para apresentações ao cliente.', updated_at=NOW() WHERE id='a9ffc0bd-70bd-dbdf-404f-8c704b3a026a';

-- 6b. 16 sistemas internos novos
...
-- 7. Melhorias: nome curto + vincular a sistema interno
