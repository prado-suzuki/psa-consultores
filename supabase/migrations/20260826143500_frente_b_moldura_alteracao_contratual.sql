...
--
-- A alteração e a constituição compartilham o mesmo modelo, mas a alteração é
-- um instrumento com duas partes: primeiro registra o que mudou e depois
-- reproduz o contrato social consolidado. Estes blocos tornam essa fronteira
-- explícita sem alterar a redação própria da constituição.
--
-- As versões são empilhadas porque documentos já validados renderizam das
-- versões congeladas no snapshot. Sobrescrever a vigente mudaria o passado e
-- retiraria do consultor a trilha editorial do catálogo.
--
...
-- ---------------------------------------------------------------------------
-- CNPJ e NIRE são condicionais porque a constituição ainda não os possui. O
-- mesmo cabeçalho continua servindo às duas peças sem deixar linhas órfãs.
DO $$
DECLARE
...
BEGIN
  IF NOT EXISTS (
    ...
    -- A vigente sai primeiro por causa do índice único parcial que garante uma
    -- só versão atual por bloco.
...
-- ---------------------------------------------------------------------------
-- IDs estáveis permitem que a Frente D altere o tipo das resoluções e marque o
-- reinício no cabeçalho da consolidação sem localizar blocos por texto ou ordem.
...
-- Cada bloco novo nasce com uma versão em vez de guardar texto no cadastro do
-- bloco. A guarda por bloco preserva qualquer histórico que já exista se a
-- migration for reaplicada sobre um catálogo parcialmente preparado.
INSERT INTO public.tmpl_bloco_versao (
  bloco_id,
  numero_versao,
  atual,
...
-- ---------------------------------------------------------------------------
-- A Frente A sintetiza exatamente uma destas flags. Assim a redação original
-- da constituição permanece intacta e só deixa de compor quando a peça é uma
-- alteração.
...
-- ---------------------------------------------------------------------------
-- `ordem` é inteira. A seção precisa ficar entre o preâmbulo (2) e as seis
-- resoluções (3..8), então as resoluções avançam uma casa. O espaço 9..12 que a
-- migration das resoluções reservou recebe a última resolução e os três blocos
-- de fechamento/reabertura. Nenhum bloco do contrato consolidado é deslocado.
--
-- O bloco de reabertura é apenas posicionado aqui. A Frente D, posterior, é quem
-- lhe dará a marca de reinício e implementará as duas séries de numeração.
DO $$
DECLARE
  v_documento record;
