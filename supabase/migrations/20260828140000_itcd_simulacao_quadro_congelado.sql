-- ════════════════════════════════════════════════════════════════════════════
-- O QUADRO CONGELADO DA SIMULAÇÃO
--
-- A simulação passa a guardar o quadro INTEIRO, e não só o que cada um doou ou
-- recebeu: com quantas quotas cada pessoa estava, quantas saíram dela, com quantas
-- termina, e em que guia o doador emite. É o registro de execução do ato.
--
-- O QUE ISSO CONSERTA. O `transmitido` era DERIVADO do rateio na hora de exibir, e
-- isso fazia o quadro antigo depender de uma convenção de arredondamento que pode
-- mudar: a mesma simulação leria números diferentes depois de um ajuste no motor.
-- Abrir uma simulação é LER.
--
-- TUDO `not null default 0`, porque nem todo donatário tem quota antes do ato — zero
-- é o caso comum, de quem entra no quadro pela própria doação. `conjuge_pessoa_id` é
-- a exceção e é anulável: só existe cónjuge na guia quando ela é conjunta.
--
-- Os percentuais NÃO têm coluna: `quotas ÷ universo` é aritmética fechada sobre dois
-- números que já estão congelados.
--
-- Idempotente: vai ser aplicada por dois caminhos (sandbox pelo CLI, produção pelo
-- chat do Lovable) e existir em duas versões.
--
-- NOTA: o cabeçalho original desta migration foi perdido numa tentativa de
-- consolidação. O SQL abaixo é o mesmo que rodou no sandbox em 28/08/2026; este
-- texto é reescrito.
-- ════════════════════════════════════════════════════════════════════════════

-- ── 1. Quem doa: o que saiu, o que ficou, e em que guia ──────────────────
alter table public.itcd_simulacao_doador
  ...
  -- Anulável, e tem de ser: só existe cônjuge na guia quando ela é conjunta.
...
begin
  ...
  -- Cônjuge sem emissão conjunta é contradição: a guia individual não tem outro
  -- nome. O contrário é possível de propósito — dado antigo, sem o vínculo.
  if not exists (
...
-- ── 2. Quem recebe: o que já tinha e com quanto termina ──────────────────────
...
-- ── 3. A regra do quadro, escrita onde ela vive ──────────────────────────────
