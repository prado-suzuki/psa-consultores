...
--   wp_revisao   cada upload: o arquivo, o checksum e a versão do mapa que leu.
...
--
-- **Revisão e valor são imutáveis de propósito.** Eles são o retrato do que a
...
create table if not exists public.wp_revisao (
  id uuid primary key default gen_random_uuid(),
  estudo_id uuid not null references public.wp_estudo(id) on delete cascade,
  versao integer not null,
  -- O arquivo reusa o acervo que já existe, em vez de um storage paralelo.
  arquivo_id uuid references public.documento_arquivo(id),
  -- Bloqueia subir duas vezes o mesmo arquivo, que é o engano mais comum.
...
comment on table public.wp_revisao is
  'Um upload do WP. Imutável: corrigir é subir de novo, gerando a revisão seguinte.';

create table if not exists public.wp_valor (
  id uuid primary key default gen_random_uuid(),
  revisao_id uuid not null references public.wp_revisao(id) on delete cascade,
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'wp_revisao_versao_unica') then
    alter table public.wp_revisao
      add constraint wp_revisao_versao_unica unique (estudo_id, versao);
  end if;

  -- A regra que o Eduardo definiu: o mesmo arquivo não sobe duas vezes.
  if not exists (select 1 from pg_constraint where conname = 'wp_revisao_checksum_unico') then
    alter table public.wp_revisao
      add constraint wp_revisao_checksum_unico unique (estudo_id, checksum);
  end if;
  ...
  if not exists (select 1 from pg_constraint where conname = 'wp_valor_celula_unica') then
    alter table public.wp_valor
      add constraint wp_valor_celula_unica unique (revisao_id, origem_celula);
  end if;
...
create index if not exists wp_revisao_estudo_idx on public.wp_revisao (estudo_id, versao desc) where excluido = false;
create index if not exists wp_valor_revisao_idx on public.wp_valor (revisao_id, bloco);
...
alter table public.wp_revisao enable row level security;
...
comment on function public.wp_estudo_visivel(uuid) is
  'Se a pessoa enxerga o estudo, e portanto as revisões e os valores dele.';
...
drop policy if exists "quem ve o estudo ve a revisao" on public.wp_revisao;
create policy "quem ve o estudo ve a revisao"
  on public.wp_revisao for select
  using (excluido = false and wp_estudo_visivel(estudo_id));

drop policy if exists "admin can view deleted wp_revisao" on public.wp_revisao;
create policy "admin can view deleted wp_revisao"
  on public.wp_revisao for select
  using (excluido = true and has_role(auth.uid(), 'admin'::app_role));

drop policy if exists "team_member+ can insert wp_revisao" on public.wp_revisao;
create policy "team_member+ can insert wp_revisao"
  on public.wp_revisao for insert
  with check (
    has_role_or_higher(auth.uid(), 'team_member'::app_role)
...
-- team_member aqui: revisão é retrato, e retrato não se edita.
drop policy if exists "admin can soft delete wp_revisao" on public.wp_revisao;
create policy "admin can soft delete wp_revisao"
  on public.wp_revisao for update
  using (has_role(auth.uid(), 'admin'::app_role));

-- wp_valor ------------------------------------------------------------------

drop policy if exists "quem ve a revisao ve o valor" on public.wp_valor;
create policy "quem ve a revisao ve o valor"
  on public.wp_valor for select
  using (
    exists (
      select 1 from public.wp_revisao r
      where r.id = revisao_id and r.excluido = false and wp_estudo_visivel(r.estudo_id)
    )
  );
...
create policy "team_member+ can insert wp_valor"
  ...
  with check (
    ...
    and exists (
      select 1 from public.wp_revisao r
      where r.id = revisao_id and wp_estudo_visivel(r.estudo_id)
    )
  );
