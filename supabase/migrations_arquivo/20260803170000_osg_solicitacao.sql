-- EDU-21 · O pedido de documentos ganha um cabeçalho.
--
-- Hoje não existe cabeçalho nenhum: a tela do consultor monta a lista em memória
-- e, no botão "Enviar solicitação", grava linha por linha em checklist_cliente_item.
-- Sem cabeçalho não dá para saber se o pedido já saiu do rascunho, quando saiu,
-- nem quando terminou. E, do lado do cliente, não há como esconder o rascunho.
--
-- Decisões fechadas em 31/07/2026:
--   - três estados: rascunho, enviada, encerrada;
--   - no máximo UMA solicitação não encerrada por cliente;
--   - o encerramento é MANUAL, por botão (ALE-30). Não existe fechamento
--     automático por completude, nem campo de progresso ou de "concluído".
--
-- Conferido no banco em 03/08/2026, não presumido: a tabela e o enum não existem;
-- cliente_visivel_para, resolve_user_cliente_id, has_role_or_higher e
-- checklist_touch_updated_at existem; cliente.id e ordem_servico.id são as PKs.
--
-- Reversão: drop table public.solicitacao; drop type public.osg_solicitacao_status;
