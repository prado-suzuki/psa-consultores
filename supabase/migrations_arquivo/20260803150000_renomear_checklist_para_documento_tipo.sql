-- EDU-20 · A palavra "checklist" sobra só no que é checklist de verdade.
--
-- Três tabelas começavam com a ideia de checklist e só uma é checklist:
--   checklist_item_padrao   = catálogo dos tipos de documento (58 linhas)  → documento_tipo
--   produto_checklist_item  = vínculo produto x tipo de documento (260)    → produto_documento_tipo
--   checklist_cliente_item  = o que um cliente deve, com status por item   → MANTÉM o nome
-- Nesta sprint entram `solicitacao` e `solicitacao_item`; sem o rename ficariam
-- cinco nomes parecidos disputando o mesmo significado.
--
-- POR QUE A FUNÇÃO É RECRIADA AQUI, e não numa migration à parte:
-- corpo de função no Postgres é guardado como TEXTO. O ALTER TABLE ... RENAME
-- atualiza sozinho FK, índice, policy e trigger (que apontam por identificador
-- interno), mas NÃO reescreve o corpo de nenhuma função. A gerar_solicitacao_os
-- cita as duas tabelas 8 vezes e continuaria apontando para nomes inexistentes.
-- Pior: a migration passaria verde e o erro só apareceria na primeira chamada.
-- O corpo abaixo é cópia fiel do que está rodando hoje (lido de pg_get_functiondef),
-- com as 8 ocorrências trocadas e nada mais. Quem reescreve a lógica é a EDU-25.
--
-- Conferido no banco em 03/08/2026, não presumido:
--   - pg_proc: gerar_solicitacao_os é a ÚNICA função cujo corpo cita as tabelas.
--   - pg_get_viewdef: NENHUMA view ou view materializada cita.
--   - a view cobertura_documentos_cliente e as RPCs get_checklist_solicitado_cliente
--     e anexar_documento_solicitado leem só checklist_cliente_item, que não muda.
--
-- Migrations antigas NÃO são reescritas: elas são o histórico do banco e já rodaram.
--
-- Reversão: os mesmos comandos na ordem inversa, mais o CREATE OR REPLACE da
-- função com os nomes antigos.
...
-- Os mesmos grants de 20260729211519: CREATE OR REPLACE preserva as permissões,
-- mas repetir aqui deixa a migration autossuficiente para um banco recriado.
...
-- O rename da tabela não renomeia trigger, índice, constraint nem policy. Nada
-- disso quebra, mas o nome passa a mentir para quem for ler depois.
...
-- constraints. As 4 de chave primária e única têm índice próprio de mesmo nome,
-- e RENAME CONSTRAINT já renomeia o índice junto: um ALTER INDEX para elas
-- falharia. As 2 de chave estrangeira não têm índice próprio.
...
-- índice solto (não pertence a constraint)
