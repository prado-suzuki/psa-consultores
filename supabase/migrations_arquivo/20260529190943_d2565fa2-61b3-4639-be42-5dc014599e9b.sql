...
CREATE OR REPLACE FUNCTION public.criar_bem_com_titular(bem_data jsonb, titular_data jsonb)
RETURNS public.bem
LANGUAGE plpgsql
...
BEGIN
  IF titular_data IS NULL OR (titular_data->>'titular_pessoa_id') IS NULL THEN
    RAISE EXCEPTION 'Titular é obrigatório para cadastrar um bem sem matrícula';
  END IF;
