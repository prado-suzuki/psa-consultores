...
--   1) Tabelas nativas Lovable (projects, processes, process_stages,
--      process_improvements, process_scenarios, job_roles) recebem APENAS
--      ADD COLUMN IF NOT EXISTS. Sem DROP, sem RENAME, sem alteração de
--      tipos.
--   2) public.tools é INTOCÁVEL (módulo Digital DEV). Sem ALTERs. Para o
--      catálogo de sistemas do MAPA, criamos public.sistemas_processo.
--   3) Digital Rotina (public.routines, public.demand_items) é interface
--      somente de export CSV. Nenhuma FK do MAPA aponta para essas tabelas.
--   4) Sem profiles, sem estrutura_equipes, sem estrutura_equipe_membros.
--      Responsáveis vivem só em public.job_roles.
--   5) Sem colunas `excluido` e `ambiente`. Sem `versao`. Sem `tipo`/
--      `pillar_code` em projects.
--   6) Português puro + FKs com sufixo `_id`. Em process_stages, a
--      auto-ref TO-BE → AS-IS chama-se `etapa_as_is_id`.
--   7) Idempotente. Envelopado em BEGIN/COMMIT.
--
-- Ordem: 0) extensões → 1) ALTERs aditivos → 2) CREATEs novos → 3) triggers
-- =====================================================================
...
-- =====================================================================
-- 1. INJEÇÕES DE ATRIBUTOS — 6 tabelas nativas Lovable
-- (public.tools é intocável — vide bloco 2.13 sistemas_processo)
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1.1  public.projects ←→ MAPA.projetos
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 1.2  public.processes ←→ MAPA.processos
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 1.3  public.process_stages ←→ MAPA.etapas (AS-IS / TO-BE)
-- Auto-ref TO-BE → AS-IS via `etapa_as_is_id` (padrão `_id`).
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 1.4  public.process_improvements ←→ MAPA.melhorias
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 1.5  public.job_roles ←→ MAPA.responsaveis
-- (ÚNICO canal de responsáveis no MAPA. Sem profiles, sem estrutura_equipes.)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 1.6  public.process_scenarios ←→ MAPA.process_snapshots
-- Histórico cronológico puro — SEM coluna `versao`, SEM UNIQUE associado.
-- ---------------------------------------------------------------------
...
-- =====================================================================
-- 2. CRIAÇÃO DE TABELAS NOVAS — snake_case em português, FKs `_id`
-- =====================================================================

-- ---------------------------------------------------------------------
-- 2.1  projeto_justificativas
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2.3  sistemas_processo (SUBSTITUI injeções em public.tools)
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.sistemas_processo (
  ...
  tipo_custo               text,                            -- 'fixo'|'variavel'|'setup'|'misto'
  obs_licenca              text,
  obs_variavel             text,
...
-- ---------------------------------------------------------------------
-- 2.4  etapa_responsaveis  (FK composta (etapa_id, cenario))
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2.5  etapa_sistemas  (FK composta + sistema_id → sistemas_processo)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2.6  etapa_documentos  (FK composta)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2.7  gargalos  (1:N com process_improvements via melhoria_id)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2.10  documento_horas_historico (append-only)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2.12  cascata_evento_etapas  (FK composta)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2.13  sistema_clusters  (sistemas_processo × estrutura_clusters)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2.14  sistema_responsaveis  (sistemas_processo × job_roles)
-- ---------------------------------------------------------------------
...
-- 3.1  CASCADE AS-IS → TO-BE em process_stages (via etapa_as_is_id).
...
-- 3.2  Trigger genérico de updated_at nas tabelas novas que carregam o campo.
