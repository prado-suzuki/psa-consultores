-- Loops dinâmicos no Contrato Social — Sociedade Limitada (Agro).
--
-- Os blocos que qualificavam sócios um a um (socio, socio2) passam a iterar as
-- fontes relacionais via seções do engine:
--   {{#socios sep="; " fim="; e "}}…{{/socios}}        → quadro_societario da empresa
--   {{#administradores sep="; " fim="; e "}}…{{/administradores}} → administracao
--   {{#sePF}}…{{/sePF}} / {{#sePJ}}…{{/sePJ}}          → qualificação PF vs PJ por item
-- Nos atributos, "\n" é escape de quebra de linha (desfeito pelo parser).
--
-- A troca respeita o versionamento: cada bloco ganha uma NOVA versão vigente
-- (numero_versao = max + 1), preservando o histórico — nada de reseed destrutivo.
