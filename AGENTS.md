...
**Botão de criar em telas de lista (padrão OSG, vale para o módulo inteiro):** a ação de criar mora no **cabeçalho do card da lista onde o item novo aparece**, como o "Novo bem" no Cadastro Patrimonial; nunca no cabeçalho da página nem dentro de card de filtros. Onde há duas criações na mesma tela (ex.: PJ e PF na Qualificação das Partes), um botão em cada card. Ação que nasce só no vazio e não volta a existir (ex.: "Criar o acordo") fica na caixa do estado vazio. Quando o botão está no card e a página tem mais conteúdo acima dele, um texto de apoio curto pode apontar onde está a ação (molde da faixa de instrução da Matriz de Alçadas). **Exceção consciente:** o Quadro Societário mantém o "Registrar movimento" no card da empresa, porque é porta única dos seis gestos societários, presa ao escopo da empresa em cena, e não criação de item de lista.

**Frame do estado vazio (padrão OSG, decisão da Patrícia de 25/09):** todo vazio de página usa o componente `@/components/shared/EstadoVazio` (`src/components/shared/EstadoVazio.tsx`), cujo frame é: largura total do card que o abriga, `rounded-xl`, **borda pontilhada de espessura única** (`border border-dashed border-osg-300`), fundo `bg-osg-50/40`, `px-6 py-12`. Nada de borda sólida, `rounded-2xl` nem padding outro em vazio de página. Contrato do componente: `titulo` (o que está vazio), `descricao` (uma linha: o que aquilo alimenta ou o primeiro passo), `acao` (obrigatória no vazio de lista, onde a regra é "todo vazio nomeia o próximo passo e o oferece"; opcional quando o vazio é de pré-requisito, como "selecione um cliente", em que a ação mora na barra). Tela nova do módulo NÃO inventa caixa de vazio própria: consome o componente ou, havendo layout próprio (disco de ícone, halo), copia as classes dele.
