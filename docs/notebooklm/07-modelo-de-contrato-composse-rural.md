# Modelo de contrato — Instrumento Particular de Composse Rural Pro Indiviso

Modelo replicável, construído cláusula a cláusula sobre o texto real de
`exemplo-01-composse-bela-vista.md` (Sérgio Pitt e Outros, 28/08/2024). Dois
pontos de variação real, confirmados por comparação direta com
`exemplo-03-composse-rossato.md` (mesma estrutura de cláusulas, família
diferente), estão marcados como blocos condicionais — ver cláusulas Segunda
(liquidação de haveres) e Décima Primeira (regra de administração). O
restante da redação fixa é idêntico nos dois contratos reais.

Mesma convenção de marcação de `06-modelo-de-contrato-parceria-rural.md`
...
{{compossuidor.naturalidade}}, nascid[o/a] em {{compossuidor.dataNascimento}},
filh[o/a] de {{compossuidor.filiacao}}, {{compossuidor.profissao}},
{{compossuidor.estadoCivil}} sob o regime de {{compossuidor.regimeBens}},
portador[a] do RG nº {{compossuidor.rg}}, inscrito no CPF/MF sob o nº
{{compossuidor.cpf}}, residente e domiciliad[o/a] n{{compossuidor.endereco}},
no município de {{compossuidor.municipio}}, Estado de {{compossuidor.uf}}
[[FIM REPETIR]], neste ato doravante denominados **COMPOSSUIDORES RURAIS** ou
simplesmente **COMPOSSUIDORES**.
...
**{{origem.letra}})** Item(ns) "{{origem.itens}}" advém[m] d{{origem.tipoInstrumento}},
[[BLOCO: origem é exploração própria]]
...
[[FIM BLOCO]]
[[BLOCO: origem vem de instrumento de terceiro]]
firmado em {{origem.dataAssinatura}}, no qual figura como Parceira Outorgante
{{origem.outorgante.denominacao}}[[SE origem.outorgante tem CNPJ]], CNPJ/MF nº
{{origem.outorgante.cnpj}}[[FIM SE]], com sede em
{{origem.outorgante.municipio}}/{{origem.outorgante.uf}}, e como Parceiros
Outorgados os COMPOSSUIDORES RURAIS,
[[FIM BLOCO]]
[[FIM REPETIR]]

*(Uma composse pode reunir imóveis de origens, datas e outorgantes
diferentes — cada alínea do "Considerando V" corresponde a um subconjunto de
imóveis do Anexo Único com a mesma origem; ver `exemplo-01-composse-bela-vista.md`,
que tem 6 origens diferentes para 15 imóveis. "Exploração própria" é o nome
usado quando quem está na composse já explorava o imóvel por conta própria,
sem contrato de cessão por trás — ver `04-clausulas-e-campos-tipicos.md`.)*
...
limitando, ao de {{culturasEAtividades}}, ou outra cultura legalmente
permitida que pretenderem explorar, nas áreas rurais descritas no anexo único
deste instrumento.
...
*(A soma das frações sempre fecha em 100% — sem cobertura parcial nos frutos
deste instrumento; ver `05-perguntas-frequentes.md`.)*
...
- **b)** o pagamento será realizado em moeda corrente nacional, através de
  ...
  [[BLOCO: liquidação mensal]]
  {{numeroParcelas}} parcelas iguais e mensais, atualizadas monetariamente
  pela variação do INPC, vencendo a primeira em 30 (trinta) dias após o
  evento
  [[FIM BLOCO]]
  [[BLOCO: liquidação anual]]
  {{numeroParcelas}} parcelas anuais e consecutivas, atualizadas
  monetariamente pela variação do INPC, vencendo a primeira em 1 (um) ano do
  evento
  [[FIM BLOCO]]
...
*(Ponto de variação real: `exemplo-01-composse-bela-vista.md` usa 60 parcelas
mensais; `exemplo-03-composse-rossato.md` usa 10 parcelas anuais — não há uma
regra padrão única entre famílias/casos diferentes.)*
...
equipamentos, implementos etc., pelo prazo de {{prazoIndivisao}}[[BLOCO: indivisão é prorrogável]],
podendo ainda ser prorrogado por igual interstício se não houver, por
escrito, {{prazoAvisoPrevio}}, o requerimento de divisão da coisa comum por
qualquer um dos COMPOSSUIDORES RURAIS; renovando-se o prazo sucessivamente,
até que formalmente uma das partes notifique a outra desejando a divisão da
coisa comum e a extinção da presente composse[[FIM BLOCO]].

*(No exemplo real, prazo de 3 anos, prorrogação automática salvo aviso até 3
...
*(É por esta cláusula que um novo participante entra numa composse
existente — nunca por uma parceria nova sobre ela. Ver
`02-contrato-de-composse-rural.md`.)*
...
*Parágrafo Primeiro:*
[[BLOCO: regra de administração por maioria]]
...
[[FIM BLOCO]]
[[BLOCO: regra de administradores nomeados]]
Locar, arrendar e/ou formar parcerias rurais em nome da COMPOSSE, e emitir
garantias a favor de terceiros (não compossuidores), só podem ser feitos em
...
*(Ponto de variação real: `exemplo-01-composse-bela-vista.md` usa a regra
"maioria dos percentuais" (3 compossuidores, todos também administradores da
outorgante original); `exemplo-03-composse-rossato.md` nomeia 2
compossuidores específicos como administradores, independente do percentual
de cada um. Não há uma regra padrão única entre os dois exemplos reais.)*
...
|---|---|---|---|---|---|---|
[[REPETIR para cada imóvel em imoveis]]| {{imovel.ref}} | {{imovel.areaCedida}} ha | {{imovel.areaTotal}} ha | {{imovel.nome}} | {{imovel.matricula}} | {{imovel.municipio}}/{{imovel.uf}} | {{imovel.proprietario}} |
[[FIM REPETIR]]

*(Estrutura confirmada em `exemplo-05-anexo-imoveis-bela-vista.md` — 15
imóveis, 6 origens, 5 proprietários diferentes numa única composse real. A
coluna "Proprietário" só varia entre imóveis quando a Composse reúne origens
de mais de um outorgante; numa Parceria, todos os imóveis do Anexo pertencem
ao mesmo outorgante — repetir o mesmo nome em todas as linhas.)*
