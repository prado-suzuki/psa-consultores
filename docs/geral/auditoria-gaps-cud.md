...
| `pages/equipe/EquipeKanban.tsx` | `useEquipeKanbanDeliverableMutations.updateStatus` | `sprint_deliverables` | update | alteração de status e `completed_at`; erro retornado pelo banco continua ignorado |
| `pages/equipe/EquipeKanban.tsx` | `useEquipeKanbanDeliverableMutations.saveDeliverable` | `sprint_deliverables` | update | salvamento integral do entregável, `completed_at` e filtro por ID preservados |
| `pages/equipe/EquipeKanban.tsx` | `useEquipeKanbanAttachments.upload` | `deliverable_attachments` | insert | metadados inseridos após upload no Storage; payload e recarga ordenada preservados |
| `pages/equipe/EquipeKanban.tsx` | `useEquipeKanbanAttachments.remove` | `deliverable_attachments` | delete | exclusão individual; precheck e remoção no Storage antes do delete preservados |
| `pages/equipe/EquipeKanban.tsx` | `useEquipeKanbanDeliverableMutations.deleteDeliverable` | `deliverable_attachments` | delete | exclusão em lote dos metadados; precheck por amostra e remoção no Storage preservados |
| `pages/equipe/EquipeKanban.tsx` | `useEquipeKanbanDeliverableMutations.deleteDeliverable` | `sprint_deliverables` | delete | exclusão do registro principal por ID após a limpeza dos anexos preservada |
| `components/equipe/dev/perdcomp/PerDetailModal.tsx` | `useInsertPerSituationDetail` | `per_situacao` | insert | atualização avulsa de situação; retorno `.select().single()` e sincronização com DW preservados |
| `components/equipe/dev/perdcomp/PerDetailModal.tsx` | `useRegisterPerReimbursement` | `per` | update | primeira etapa do ressarcimento; valores atualizado/original, arredondamento e filtro por `nr_per` preservados |
| `components/equipe/dev/perdcomp/PerDetailModal.tsx` | `useRegisterPerReimbursement` | `per_situacao` | insert | segunda etapa sequencial do ressarcimento; situação `PER deferido`, `dt_pagamento` e retorno `.select().single()` preservados |
| `components/equipe/dev/perdcomp/PerDetailModal.tsx` | `useClearPerReimbursement` | `per` | update | limpeza do ressarcimento; `vlr_ressarcido` e `vlr_ressarcido_original` definidos como `null`, metadados de atualização e filtro por `nr_per` preservados |
| `components/equipe/dev/perdcomp/DcompFormModal.tsx` | `useCreateDcompForm` | `dcomp` | insert | verificação de duplicidade por `nr_documento`, payload em array e sequência antes da substituição do rateio preservados |
| `components/equipe/dev/perdcomp/DcompFormModal.tsx` | `useUpdateDcompForm` | `dcomp` | update | payload, ausência de precheck preexistente e filtro pelo `nr_documento` original preservados |
| `components/equipe/dev/perdcomp/DcompFormModal.tsx` | `useCreateDcompForm` / `useUpdateDcompForm` | `distribuicao_dcomp` | delete | substituição integral do rateio; leitura amostral best-effort, precheck quando há ID e filtro por `nr_documento` preservados |
| `components/equipe/dev/perdcomp/DcompFormModal.tsx` | `useCreateDcompForm` / `useUpdateDcompForm` | `distribuicao_dcomp` | insert | inserção em lote após o delete; cálculo ou preservação de `valor_original` e ausência de insert para lote vazio preservados |
| `pages/equipe/EquipeDemandas.tsx` | `useEquipeDemandaParentMutations.createRoutineMutation` | `routines` | insert | payload e normalizações legadas preservados; sem precheck |
| `pages/equipe/EquipeDemandas.tsx` | `useEquipeDemandaParentMutations.updateRoutineMutation` | `routines` | update | payload, filtro por ID e propagação de erro preservados; sem precheck |
| `pages/equipe/EquipeDemandas.tsx` | `useEquipeDemandaParentMutations.deleteRoutineMutation` | `routines` | delete | filtro por ID e propagação de erro preservados; sem precheck |
| `pages/equipe/EquipeDemandas.tsx` | `useEquipeDemandaParentMutations.updateRoutineStatusMutation` | `routines` | update | alteração de status e filtro por ID preservados; sem precheck; campo `error` intencionalmente ignorado |
