...
| `o-azul-de-tres-empregos.html` | as 100 cruas de `blue`: qual delas é papel, qual é categoria, e qual não é cor nenhuma | **em aberto.** Divide o lote em seis grupos, sem sobra, e mostra por que varrer por família seria pior aqui do que foi no cinza: o azul é a **única família com três casas legítimas no contrato** — `fila` (papel, já azul), `tag-b` (o frio da área) e `info` (semântico que **não** acompanha a área). Nove ocorrências já têm resposta escrita em outro arquivo do repositório, 22 são o mesmo bloco copiado 4 e 3 vezes, e quatro decisões param em você. Mede dois contrastes que ninguém tinha medido: `blue-600` sobre `blue-500/15` no selo do `BalanceteTreeTable` dá **4,35:1**, e — pior, porque é token — `info` sobre `info/10` no `taskPriorityColors.medium` dá **4,49:1**. Os dois reprovam AA. |
