...
| [`planos/lista-de-tarefas-texto-e-prazo.md`](planos/lista-de-tarefas-texto-e-prazo.md) | 🟡 PARCIAL | Feedback do Welber de 09/09 sobre a Lista. **Fase 1 feita e validada** (título em duas linhas e tooltip nas quatro linhas da grade). **Fase 2 quase inteira**: a recusa por prazo vale nos dois sentidos em `useOrgTasks`, com a redação da Patrícia em `src/lib/orgTaskPrazo.ts`; **falta só o calendário da linha da Lista**, que esperou a Fase 5 do plano do celular sair do arquivo. **Fase 3 aberta** (trigger em `org_tasks`, depende de migration) |
