...
| [`osg/doacao-de-quotas-com-usufruto.md`](osg/doacao-de-quotas-com-usufruto.md) | 🟡 PARCIAL | Doação de quotas com reserva de usufruto, em quatro fatias. **Fatias 1, 2 e 3 entregues em 10/09/2026** (migrations `20260910202720`, `20260910212126` e `20260910220109` aplicadas no sandbox; produção pendente): o evento entra no livro pelo Quadro Societário, com o ônus em `onus_quotas` como estado da sociedade; a peça deriva `evento_doacao_quotas` e compõe as cinco resoluções do ato; e o contrato consolidado republica o ônus vigente em três pontos fixos, com as três somas da tabela de voto conferidas antes de publicar. **A redação das cláusulas ainda precisa do aceite jurídico.** **Fatia 4 parcial**: a sub-rogação do gravame e a instituição de usufruto avulsa entram como FATO (gesto "Instituir usufruto" no Quadro Societário, ônus seguindo a quota no registro de movimento, reverter ressuscitando o ônus) e o consolidado já as publica; falta a resolução da instituição na peça, travada pela decisão aberta nº 4 do doc (como a peça sabe que uma instituição está pendente, se ela não tem movimento para carimbar) |
...
| [`planos/2026-09-03-modelo-de-documento-na-solicitacao.md`](planos/2026-09-03-modelo-de-documento-na-solicitacao.md) | 🟡 PARCIAL | Documento que a PSA manda em branco para o cliente preencher (planilhas). **Entregue em 08–09/09**, conferido em produção: bucket `osg-modelos`, as 3 colunas de `documento_tipo`, as duas RPCs do portal e o botão de baixar nas três telas (analista, gaveta e checklist). Só 2 dos 68 tipos têm modelo hoje. **Falta** o filtro de OS por Empresa/Faturamento que o mesmo documento mede (17 OS invisíveis) |
...
| [`sprints/sprint-12/TAREFA_mensagens-de-recusa.md`](sprints/sprint-12/TAREFA_mensagens-de-recusa.md) | 🟡 PARCIAL | T1–T5 concluídas em 02/09 com testes; D1–D5 fechadas. A chave `RECUSA_DE_ESCRITA_E_SO_POR_CARGO` **virou `true` em 14/09/2026**, depois de conferido por SELECT que nenhuma policy de escrita do cadastro cita cluster em produção: a recusa de gravação voltou a dizer qual papel falta, em vez de cair em Falha genérica. **Falta a T6, a conferência da Patricia** — e junto dela as duas frases das travas de exclusão de OS (tarefa 5 da sprint 13), cujos detalhes foram reescritos porque os sugeridos mandavam desvincular, e desvincular não existe em nenhum dos dois casos |
| [`sprints/sprint-13/README.md`](sprints/sprint-13/README.md) e as **5 tarefas** de permissão do cadastro de cliente | ✅ FEITO | Auditoria das 32 operações do módulo (02/09/2026). As **nove migrations do bloco estão em produção**, conferido por SELECT em 12/09: as três colunas `excluido` caíram, nenhuma policy de escrita pede cluster e a trava da solicitação está em `RESTRICT`. Validado na tela em 11/09, com navegador real: as quinze telas que leem essas tabelas carregam, a OS apaga levando rateio e produtos, as duas travas recusam com frases distintas, e a exclusão de contribuinte e de representante foi confirmada no banco. **Uma décima migration fecha os gaps**, `20260911212357_alcadas_cadastro_lider_e_cluster`, aplicada no sandbox em 12/09 e **pendente em produção**: a matriz de alçadas foi validada com a coordenação em 11/09 e padronizou OS, Faturamento e Proposta em `lider`+, passou a leitura de representante e a trilha de auditoria a filtrar por cluster, e fez as funções `SECURITY DEFINER` de exclusão lógica conferirem cluster por dentro — elas rodam fora das regras de permissão, e ali a leitura não protege. O front acompanha: as regras de papel saíram para `src/lib/cadastroClientePermissions.ts`, o botão Excluir passou a valer para `sublider`+ e `useDeleteCliente` deixou de fazer `update` direto e chama a função. **Fica de fora, por decisão:** a aba Faturamento é só leitura e não ganha escrita, e o usuário-cliente segue vendo a própria proposta pelo portal |
...
| [`planos/cadastro-produtos-e-servicos.md`](planos/cadastro-produtos-e-servicos.md) | ✅ FEITO | **Entregue em 14/09/2026**, quatro commits, cada um validado visualmente pela Patrícia antes do seguinte. O cadastro de serviço ganhou campos **Número** e **Nome** separados, com o próximo código livre já proposto e aviso de código/nome já existentes — tudo gravado na mesma coluna `nome`, no mesmo formato, **sem migration**, porque ela recusou coluna nova antes de arrumar o formulário. A tela perdeu o `AcoesEmMassaMenu`, a terceira coluna de 320px (virou `Sheet`), os cartões de Acessos nesta aba e o único breakpoint que tinha. Ganhou "Copiar de outro produto". **Abrir para o diagnóstico**, que é o que não está no código: 25 códigos distintos para 68 serviços numerados no TAX, 5 pares de serviço duplicado com os vínculos partidos entre as duas linhas, e 273 tarefas com o número no título. ⚠️ O commit do responsivo **não existe** — uma sessão paralela o engoliu em `e28fce6a`. Nada foi para produção e nada aqui é migration |
...
| [`geral/cor-o-que-falta.md`](geral/cor-o-que-falta.md) | 🔵 ABERTO | **O ponto de retomada da frente de cor.** Atualizado em 03/09, com o motivo de cada parada. **Fecharam** o §4 inteiro (rótulos de chamado, e a palavra única masculina dos três pares), o estado de documento — que virou o sexto mapa do contrato — e o `osg-red` do §7, com catraca própria, e a pasta `equipe/audit`. A lista "se você for retomar por um só item" já começa no próximo, e diz por que `red`/`emerald` NÃO são conversão por mapa — o cabeçalho registra que a alavanca de mapa rendeu cinco vezes em cinco, e a varredura nenhuma. Contém o achado de `projects.status`, que **não é dívida de cor e sim defeito que o cliente vê**: três mapas, nenhum casa com o dado, e todo projeto aparece como "Em Planejamento" a 0%. Em 10/09 fecharam a **tela `/equipe/dev`** e as **seis rotas de hub**, e com elas veio uma unidade nova de conferência — por ROTA, não por família nem por mapa: 28 classes cruas numa página que o lint dava por limpa, com catraca de tela própria. Nas rotas de hub, dois componentes compartilhados fecharam seis rotas, e a caixa "Visão Geral" (vinte telas) tinha o `--accent-soft` escrito como hex e um link reprovando AA a 3,38:1. `accent-d`/`accent-soft` ganharam classe no `tailwind.config.ts`. Ainda em 10/09, o **`--muted` deixou de ser escrito à mão** nas três áreas — `rebaixar(--canvas)` o gera, com catraca que **recalcula** em vez de olhar — e a pilha de matizes da Tax fechou. O §6 parou de dizer só "o resto dos tokens escritos à mão" e passa a listar, token a token, o que continua manual e o que falta decidir para deixar de ser: `border`/`input` foram fechados no fim do mesmo dia. **A FRENTE DAS SUPERFÍCIES ENCERROU em 10/09:** página, `--muted`, `--border`, `--input`, título e nome de área saíram todos da mão, cada um com catraca própria, e a dívida da WCAG 1.4.11 foi paga sem token novo — o `--input` já era a linha de controle, só tinha o valor do `--border`. **A lista de retomada foi remedida e mudou de cabeça:** ela mandava começar por `red`/`emerald` e a maior é o `gray`, com 580 em 61 arquivos, que nunca esteve nela; e ele é conversão por MAPA (12 arquivos concentram 313) e não inventário. O bloco agora traz o comando que produz a contagem. **Em 10/09 o `gray` FECHOU**: de 580 para 149, nenhuma em tela interna fora do bloco de código, com catraca `filaDoGray.test.ts` (inventário por motivo mais asserção própria do recorte interno). O site público ficou de fora por decisão dela — a landing pinta a própria paleta e ali o cinza claro sobre escuro está certo. **Em 11/09 o `teal` FECHOU**, e a família inteira zerou nas pastas de tela (não só os três degraus do aviso), com catraca `filaDoTeal.test.ts` e a regra de ESLint subindo de `warn` para `error`. A frente foi aberta com o argumento da MATIZ — "quando `/equipe/acessos` virou grafite, os botões continuaram teal" — e medindo, esse argumento alcançava **5 das 115** ocorrências: só `.tax-theme` e `.osg-theme` reescrevem `--primary`, e nas outras áreas `--primary` é `175 84% 25%`, o mesmo número do `--teal-600`. O que havia de verdade era degrau: 45 no 600 (pixel idêntico), 15 no 500 (escurece) e 55 no 700 (clareia, 7,55:1 → 5,54:1) — e foi esse último que a usuária validou olhando, decidindo aceitar a perda em vez de criar um token semântico no degrau 700. Três achados que a varredura por família não teria dado: pares que **colapsam** quando dois degraus viram um token (`bg-teal-600 hover:bg-teal-700` vira hover morto em 4 botões, cujo conserto é apagar a classe e usar a variante `default` do `ui/button`; `from-teal-600 to-teal-700` vira gradiente liso); `utils/pdf/theme.ts` tem `#0d9488` rotulado `// teal-600` e **o rótulo está errado** — é o teal do Tailwind, não o `#0A756C` institucional; e `client-form/acentoArea.tsx` era um mecanismo MANUAL de acento por área (contexto React escolhendo entre `TEAL` e `OSG`) que se justificava por portal não herdar tema, premissa que o `index.css:952` contradiz. **Fechado no mesmo dia, e a medição corrigiu a primeira leitura:** ele NÃO era inteiramente substituível por `--primary` — só metade era. Os cinco papéis de acento colapsavam (o mapa OSG cravava `text-osg-moss`, e sob `.osg-theme` o `--primary` É `var(--osg-moss)`, a mesma variável), mas os quatro de superfície carregavam informação real que nenhum tema declara: a OSG tem superfície QUENTE com âncora verde, e `bg-accent/5` sobre a areia dela dá `#F1F3F0`, cinza frio, contra `#EBE3DB`. Eram duas saídas legítimas — criar token de superfície por área, ou aceitar o token comum — e a usuária escolheu a segunda olhando o modal montado dos dois jeitos: modal é superfície elevada, e areia sobre areia não elevava. O arquivo saiu inteiro, 30 usos em 11 telas viraram token direto, e o `<html>` da OSG não perdeu uma linha (`osg-50` segue em 158 lugares, `osg-100` em 132) **Em 11/09 o `red` e o `emerald` FECHARAM**: de 248 ocorrencias para 25, com catraca `filaDoRedEmerald.test.ts` — inventario por motivo mais uma assercao de que nenhuma tela interna tem vermelho ou verde cru. O que sobra e site publico (10), paleta categorica (8) e rotulo de cargo (7), nenhum deles estado. As decisoes foram todas dela, olhando: a pagina `comparacoes-de-cor/vermelho-e-verde-o-que-cada-um-diz.html` (reescrita no fim do dia para conter so o que ficou aberto), a tela rodando com os candidatos aplicados ao vivo, e um artefato de tres cliques para as ultimas tres. **Em 11/09 o `blue` fechou a rodada 1**: de 100 ocorrências em 39 arquivos para **52** em 25, com catraca `filaDoBlue.test.ts` em cinco grupos pelo motivo. Duas lições que não valem só para ele. **Medir a classe não é medir o que aparece:** registrei um selo como reprovando o AA (4,35:1 "em 10px negrito") e o conteúdo dele era um `<Check/>` — o piso de ícone é 3:1 e ele passava; o achado que sobrevive é em TOKEN, `bg-info/10 text-info` a 4,49:1 sob a palavra "Média". **E as ROTAS dizem o tamanho do commit, a pasta não:** percorrendo os `import` até o `App.tsx` apareceram cinco ocorrências em código que nenhuma rota monta, e uma linha só — o ícone de documento do `docMeta` — com alcance de doze rotas. A rodada 2 está travada numa decisão dela: o que sobra é quase todo mapa de CATEGORIA com mais entradas do que o contrato tem tons (10, 6, 6), e a paleta de oito (`--area-1..8`) colapsa sob daltonismo quando a cor aparece sozinha. **Em 12/09/2026 abriu a frente das SUPERFÍCIES DE OBJETO, e ela é filha da página branca:** com a página branca, cartão e campo ficaram no mesmo valor, e o que separava os três era uma linha a 1,23:1. A varredura das **96 rotas logadas** desfez a leitura intuitiva — o Dashboard tem o MESMO percentual de branco do Feed (70 x 71%), e o que o Feed tem é tinta ENTRE as caixas; "branco dentro de branco" é a regra do sistema (36 telas ≥ 85% sem caixa tingida) e o Feed é a exceção. A opção C põe a tinta no OBJETO, com a receita do Feed e nenhuma cor nova. **Entregue:** o `<Card>` tingido alcança 392 usos em 173 arquivos numa linha, o `KpiHero` junto, e das **101 caixas escritas à mão em 72 arquivos** (25 na OSG) **70 foram tingidas** e 31 ficam brancas por motivo inventariado — flutua sobre conteúdo, é controle, apoia-se no rebaixado, é conteúdo dentro do cartão, é papel, ou é o aviso já decidido em `b63d6ede`. O token `--card` **não mudou de valor**, e é o ponto: ele pinta onze cabeçalhos e barras laterais, `SelectTrigger`, `Input` e modal. Catraca `cartaoTingido.test.ts`, que lê a EXPRESSÃO de classe e não a linha (o `KpiHero` prova por quê: `rounded-2xl` e `bg-card` em linhas diferentes do mesmo `cn()`). Achado de método: **cinco `<Card className="bg-card">` cancelavam a tinta em silêncio**, e a regra de ESLint `ui/token-nao-sobrescrito` não os pegava de propósito — ela libera sobrescrita para OUTRO token, e `bg-card` é outro token. **Aberto, e é decisão dela:** a caixa de TABELA (19 telas, e se a saída for "tabela branca" são 52 blocos `<Card>` com tabela dentro, por variante do componente); o Board, que pinta pelo CSS próprio (`--bd-surface` = `hsl(var(--card))`) e ficou branco; a centralização das duas telas de coluna solta; e os filtros em barra. **Em 12/09 fechou também o DEGRAU SOBRE O CARTÃO, e ele é o defeito de classe da tinta:** quando uma superfície se move, todo degrau construído sobre ela se move junto, e nada falha. O hover de linha e a faixa de totais do `ui/table` caíram de 1,112 / 1,120 / 1,106 para 1,073 / 1,078 / 1,069 nas três áreas no dia em que o cartão desceu, sem uma linha de código mudar. Os **66** `bg-muted/50` escritos à mão foram classificados pelo FUNDO e não pela classe: **41 se apoiam em cartão** e viraram `bg-superficie-realce` (`hsl(var(--muted) / 0.75)`, o alfa que recompõe os três números exatos), e **25 ficam**, porque modal e gaveta são `bg-background` e popover é `bg-popover` — converter os 25 junto teria escurecido caixa que está certa, o erro simétrico do que a conversão consertou. Catraca nas duas direções no `cartaoTingido.test.ts`: uma RECALCULA o degrau a partir do `index.css` e do `tailwind.config.ts`, a outra é o inventário dos 25 agrupado por motivo. **E abriu frente nova, achada medindo outra coisa:** `bg-white` cru em caixa arredondada não é visto por nenhuma das duas catracas — a do cartão procura `bg-card`, e a de cor crua só dispara em sobrescrita de componente do `ui/`, não em `div` solta. São **142 ocorrências em 79 arquivos** das pastas de tela, nenhuma medida ainda. |
...
| [`geral/matriz-de-acessos-editavel.md`](geral/matriz-de-acessos-editavel.md) | ✅ FEITO | **14/09/2026, entregue na `develop`, sem migration.** A seção Papéis de `/equipe/acessos` deixa de ser só-leitura: a célula grava no clique (Desfazer de 10s) e a seleção aplica papel, área e equipe em lote. Isso **derruba** o eixo que `secoesDeAcessos.ts` registrava — não é mais leitura x escrita, é "uma pessoa, tudo dela" x "todo mundo, uma dimensão só". Duas decisões dela: escopo com equipe em lote, e gravar na hora em vez de acumular até um "Salvar" (que devolveria metade do custo que abriu a frente). O que faz o Desfazer não ser destrutivo é o recorte de `alterados`: a mutação lê o estado antes e reverte só quem de fato mudou — marcar `admin` em 12 das quais 9 já eram admin e desfazer não tira o papel das 9. Duas travas novas: ninguém remove o próprio `admin` (na mutação, não no botão), e a seleção não alcança quem o filtro tirou da tela. **Achado no caminho, e é defeito que a tela mostrava:** o card "Permissões Customizadas" exibia **1000** e produção tem **1494** — o PostgREST corta em 1000 linhas **sem avisar**, e um terço dos vínculos era invisível a qualquer leitura global; o docstring do hook já chamava isso de "atenção", o que deixou a promessa da assinatura falsa na tela. Vale além desta tabela: teto de paginação não parece erro, parece número redondo. **Da responsividade, a medição que importa:** o ponto de virada não é a viewport e sim a COLUNA — a partir de `md` a barra volta ao fluxo e leva 256px, então em 768px cravados os três grids disparavam com 464px de coluna (116px por cartão). E a tabela tinha dez colunas porque as pílulas e as sete colunas de ✓/✗ são a mesma informação duas vezes. **A dimensão de EQUIPE entrou depois, a pedido dela, e trouxe o segundo defeito:** eu havia descartado a coluna dizendo que a estrutura é árvore e não matriz — medindo, são 11 equipes e cabem; o argumento valia para o seletor, não para a coluna. Ao montá-la apareceu o que a captura de tela dela já mostrava sem que ninguém tivesse reparado: o chip do diálogo exibia `32bc9000-f524-43f0-9aa9-44d2381c17d7`, que é a equipe "Área para Estudos e Pesquisas" — **desativada, com 9 membros**. Desativar equipe não desliga ninguém: são **15 vínculos em três equipes desativadas** em produção, e para todos eles `caminhoDaEquipe` devolvia `null` e o `?? equipeId` imprimia UUID. A causa é confundir OFERECER com MOSTRAR, e agora são duas listas — `montarGruposDeEquipe` (só ativas, para entrar) e `colunasDeEquipeDaMatriz` (ativa **ou** com gente dentro, para ver e sair). **ORDENAÇÃO POR CLIQUE** entrou depois (crescente → decrescente → padrão; o terceiro clique existe porque tabela sem desfazer obriga a recarregar a página). O ganho não é conveniência: clicar numa COLUNA DA MATRIZ junta quem tem aquilo no topo, e por isso o primeiro clique numa coluna de ✓ é "quem tem primeiro" e não A→Z. **A largura em produção obrigou a duas mudanças que o sandbox não mostrava** (ele tem 8 equipes, produção tem 11): o e-mail sai na dimensão de equipe, e o caminho da área trunca enquanto o nome não — "TAX › Trabalhos compartilhados OSG" mede 187px contra 52px de "Fiscal", e era o contexto repetido que estourava. Medido numa tabela real de 11 colunas: 1574px → 1313px. **Em aberto:** (1) as 15 pessoas em equipe desativada — mudam de equipe, ou a equipe volta a ser ativa? (2) **abaixo de ~1650px a matriz de Equipes rola e a coluna do nome sai da tela** — a saída é `sticky`, que esbarra no contrato de cor (célula fixa pede fundo opaco, e o cartão é `--muted` a 35%). **(3) A pasta `administracao/` é código morto desde 13/01/2026**, achado ao validar: as três rotas `/administracao/*` dão **404** — a rota saiu do `App.tsx` no commit que criou o `/gestao` (`68356afc`) e as páginas ficaram sem ninguém montar, com o `AdminLayout` ainda apontando um menu para elas. Consequência: a `UsersRolesView` tem **um consumidor só** e o docstring dela afirmava dois — *docstring que lista consumidor envelhece sem avisar; esse sobreviveu oito meses a duas rotas mortas*. Apagar são 685 linhas em 4 arquivos **mais cinco catracas** que os citam no inventário (`filaDoAlerta`, `filaDoBlue`, `filaDoRedEmerald`, `sidebarMedidas`, `fundoDePagina`) e dois hooks órfãos com teste. A pergunta que decide: essas telas voltam? |
