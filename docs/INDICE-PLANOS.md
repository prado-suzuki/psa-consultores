...
| [`sprints/sprint-13/README.md`](sprints/sprint-13/README.md) e as **5 tarefas** de permissão do cadastro de cliente | 🟡 PARCIAL | Auditoria das 32 operações do módulo (02/09/2026). Regra decidida: gravar exige só cargo `sublider`+; ler segue por cluster; linha excluída só em `cliente` e `contribuinte`. **As 9 migrations estão aplicadas no sandbox** e carimbadas no ledger, auditadas uma a uma. As três últimas (`20260910201119`, `...121`, `...123`) apagam dado e derrubam a coluna `excluido` de `representante`, `ordem_servico` e `distribuicao_receita`; a ordem entre elas importa — a limpeza dos órfãos vem antes do drop. A de representante falhou na primeira tentativa por uma **sexta policy que não está em nenhuma das duas tabelas**: `Clients can read their cliente_clusters`, em `cliente_clusters`, lê `representante.excluido` num subselect. Só `pg_depend` acha; procurar policy nas tabelas alvo não encontra. **Produção não recebeu nada** e não recebe por aqui: é passo humano pelo chat do Lovable. ⚠️ **A ORDEM DO DEPLOY IMPORTA:** o front já está escrito supondo as migrations aplicadas — ele chama `.delete()` em representante, OS e rateio. Se o front subir para produção **sem** as migrations, essas policies ainda pedem cluster, o delete atinge 0 linhas e **o salvamento do cadastro falha na cara do consultor**. Migrations primeiro, ou as duas coisas no mesmo deploy. **11/09: validado na tela**, com navegador real, trocando o papel de uma conta entre as medições. As 15 telas carregam sem a coluna (o risco dos 21 filtros removidos está liquidado); a OS apaga com cascata levando rateio e produtos; as duas travas recusam com frases distintas; e a exclusão de contribuinte e de representante pela tela foi confirmada no banco. Gaps que nenhuma tarefa cobre, todos no front: o modal mostra **4 abas** para `team_member` e `sublider` contra **7** para `lider` e `admin`, e sem a aba de OS o sublíder não alcança a tarefa 5; o botão Excluir é renderizado para sublíder e recusa no clique. Gap de banco: as funções da tarefa 3 rodam fora das regras de permissão (`SECURITY DEFINER`) e não conferem cluster — é a única aba em que a leitura não protege a gravação. Achado à parte, de texto: ao remover contribuinte ou representante o salvamento grava e mesmo assim responde "Nenhuma alteração detectada", mandando confirmar um botão que não existe (`useSaveClientTransaction.ts`, `nothingChanged` não conta remoção de item) |
