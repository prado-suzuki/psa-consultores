...
| [`osg/doacao-de-quotas-com-usufruto.md`](osg/doacao-de-quotas-com-usufruto.md) | 🟡 PARCIAL | Doação de quotas com reserva de usufruto, em quatro fatias. **Fatias 1 e 2 entregues em 10/09/2026** (migrations `20260910202720` e `20260910212126` aplicadas no sandbox; produção pendente): o evento entra no livro pelo Quadro Societário, com o ônus em `onus_quotas` como estado da sociedade, e a peça deriva `evento_doacao_quotas` e compõe as cinco resoluções do ato (doação, reserva de usufruto, gravames, renúncia à preferência, quadro de usufruto e voto). **A redação das cláusulas ainda precisa do aceite jurídico.** **Abertas**: os três ecos do gravame na consolidação (3) e as variantes de cessão gratuita e instituição avulsa (4) |
