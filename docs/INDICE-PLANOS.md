...
| [`planos/lista-de-tarefas-texto-e-prazo.md`](planos/lista-de-tarefas-texto-e-prazo.md) | 🟡 PARCIAL | Feedback do Welber de 09/09 sobre a Lista. **Fases 1 e 2 feitas** — título em duas linhas com tooltip, e o prazo da subtarefa travado no da mãe nos dois sentidos (`src/lib/orgTaskPrazo.ts` guarda a redação da Patrícia; a recusa mora em `useOrgTasks`, que lê a mãe no banco). **Falta a Fase 3**: trigger em `org_tasks` para a regra valer também fora da tela — depende de migration, e produção tinha 35 pares fora da regra em 09/09 |
