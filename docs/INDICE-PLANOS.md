...
| [`planos/lista-de-tarefas-texto-e-prazo.md`](planos/lista-de-tarefas-texto-e-prazo.md) | 🟡 PARCIAL | Feedback do Welber de 09/09 sobre a Lista. **Fase 1 feita** (título em duas linhas e tooltip nas quatro linhas da grade); **faltam a 2** (calendário não oferece data depois do prazo da mãe, mais o guard no hook — a redação das mensagens é da Patrícia) **e a 3** (trigger em `org_tasks`, depende de migration). Medido em produção em 09/09: 35 de 204 pares mãe/filha fora da regra, e a tarefa mediana (37 caracteres) não cabia na coluna |
