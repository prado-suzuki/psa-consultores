...
| [`planos/projetos-tarefas-no-celular.md`](planos/projetos-tarefas-no-celular.md) | 🟡 PARCIAL | **Fases 1 a 5 feitas, 6 a 8 abertas.** Uma tela só (`PainelTarefas`) servindo 4 rotas de Tax e OSG. Roda uma fase por commit, com validação visual da Patrícia entre uma e outra. A ressalva do arrastar no Kanban está **decidida**: no celular a tela é só leitura, então não abrir frente de biblioteca de DnD |
| [`planos/lista-de-tarefas-texto-e-prazo.md`](planos/lista-de-tarefas-texto-e-prazo.md) | 🟡 PARCIAL | Feedback do Welber de 09/09 sobre a Lista. **Fases 1 e 2 feitas e validadas** — título em duas linhas com tooltip, e o prazo da subtarefa travado no da mãe nos dois sentidos (redação da Patrícia em `src/lib/orgTaskPrazo.ts`, recusa em `useOrgTasks`, que lê a mãe no banco). **Fase 3 escrita e não aplicada**: a migration `20260909173700_org_tasks_prazo_dentro_da_mae` existe no repo e **não rodou em banco nenhum** — sandbox é `db:sync --apply` pela Patrícia, produção é passo humano pelo chat do Lovable |
