...
| [`sprints/sprint-12/TAREFA_mensagens-de-recusa.md`](sprints/sprint-12/TAREFA_mensagens-de-recusa.md) | 🟡 PARCIAL | Catálogo de textos fechado e implementado em `src/lib/rlsMessages.ts`. A chave `RECUSA_DE_ESCRITA_E_SO_POR_CARGO` segue `false` de propósito — vira `true` quando as três migrações da sprint 13 estiverem em produção |
...
| [`geral/avisos-prazo-tarefa.md`](geral/avisos-prazo-tarefa.md) | 🟡 PARCIAL | Avisos de prazo de tarefa no sino (GES-01A). **Retrato do texto de hoje, aguardando a redação da Patrícia** — o texto atual é de trabalho e nunca foi revisado. Só no sandbox: produção não tem os dois tipos novos do enum. Traz seis pontos a decidir, entre eles o rótulo dos tipos novos no sino, que hoje sairia "Aviso" genérico |
