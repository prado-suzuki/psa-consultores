...
| [`planos/projetos-tarefas-no-celular.md`](planos/projetos-tarefas-no-celular.md) | 🟡 PARCIAL | **Fases 1 a 4 feitas, 5 a 8 abertas.** Uma tela só (`PainelTarefas`) servindo 4 rotas de Tax e OSG. Roda uma fase por commit, com validação visual da Patrícia entre uma e outra. A ressalva do arrastar no Kanban está **decidida**: no celular a tela é só leitura, então não abrir frente de biblioteca de DnD |
| [`planos/lista-de-tarefas-texto-e-prazo.md`](planos/lista-de-tarefas-texto-e-prazo.md) | 🔵 ABERTO | Feedback do Welber de 09/09 sobre a Lista: título que corta, tooltip só na linha de projeto e prazo de subtarefa sem trava. **Medido em produção**: 35 de 204 pares mãe/filha já fora da regra, e a tarefa mediana (37 caracteres) não cabe na coluna. Três fases, a 3ª depende de migration. Conflita com a Fase 5 do plano do celular — está anotado nos dois |
