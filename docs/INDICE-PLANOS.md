...
| [`geral/comparacoes-de-cor/LEIA.md`](geral/comparacoes-de-cor/LEIA.md) | 🟡 PARCIAL | **Onze páginas: seis já decidiram, cinco seguem em aberto** — porta de entrada, superfície de estado, o azul, o vermelho e o verde (170 de 248 decididas) e a cor que viaja como dado. As páginas HTML são autocontidas — abrir no navegador. A de 11/09, `cor-que-viaja-como-dado.html`, junta as duas frentes em que a cor é **dado que viaja** (o mapa copiado do §1 e o `${cor}NN` do §5) e traz três medições que mudam o tamanho delas: **três ocorrências de alfa colado já estão quebradas no ar** (e em `border-color` o defeito aparece como borda **opaca demais**, não como fundo ausente — a declaração vira `unset`, que é `currentColor`), a cópia do badge de papel que **reprova AA em 4 dos 7** é a que está na lista de usuários, e **a calculadora IBS/CBS nunca dependeu da decisão 4** — `html-to-image` existe num arquivo só, com um consumidor |
